...
# ============================================================
#  GAMEPAD (JOYSTICK) DESTEĞİ
#  Her oyuncu KENDİ bilgisayarında kendi oyununu çalıştırdığı için
#  (multiplayer'da bile), "gamepad+gamepad" ekstra bir eşleştirme
#  gerektirmiyor — herkes kendi tarafında istediği kontrolcüyü kullanır.
#  Bu yüzden burada her zaman sadece 0. index'teki (ilk takılı) gamepad'i
#  okuyoruz; klavye/fare de HER ZAMAN aynı anda çalışmaya devam ediyor
#  (biri diğerini geçersiz kılmıyor, ikisi birlikte kullanılabiliyor).
pygame.joystick.init()
GAMEPAD_OLU_BOLGE = 0.25  # analog stick'in ufak titremelerini yok say

def gamepad_al(indeks=0):
    """indeks'teki gamepad'i döndürür, yoksa None. Her karede güvenle çağrılabilir
    (oyun sırasında takılıp çıkarılmayı da tolere eder). Yerel 2 kişilik multiplayer'da
    indeks=0 P1'in, indeks=1 P2'nin kontrolcüsü olarak kullanılır."""
    try:
        if pygame.joystick.get_count() > indeks:
            j = pygame.joystick.Joystick(indeks)
            if not j.get_init():
                j.init()
            return j
    except Exception:
...
def fare_konumu_ham():
    """Gamepad'i hiç dikkate almadan, SADECE gerçek fare konumunu döndürür.
    Yerel 2 kişilik modda klavye+fare kullanan oyuncu için gerekli — böylece
    diğer oyuncunun gamepad'i sağ stick'i oynatınca bu oyuncunun nişanı bozulmaz."""
    mx, my = pygame.mouse.get_pos()
    olcek, ofset_x, ofset_y, _, _ = _olcek_bilgisi()
    return (int((mx - ofset_x) / olcek), int((my - ofset_y) / olcek))

def gamepad_hareket_x(j):
    """Sol stick ya da D-pad'den yatay hareket değeri (-1, 0, 1) döndürür."""
    if j is None: return 0
    try:
        eksen_x = j.get_axis(0)
        if abs(eksen_x) > GAMEPAD_OLU_BOLGE:
            return 1 if eksen_x > 0 else -1
        if j.get_numhats() > 0:
            hat_x, _ = j.get_hat(0)
            if hat_x != 0: return hat_x
    except Exception:
        pass
    return 0

def gamepad_ziplama_tetiklendi(j, onceki_basili):
    """A/Cross (buton 0) tuşuna YENİ basılıp basılmadığını (kenar algılama) döndürür.
    Dönüş: (yeni_basildi_mi, guncel_basili_durumu) — ikinci değeri bir sonraki karede tekrar geçir."""
    if j is None: return False, False
    try:
        simdi = bool(j.get_button(0))
        return (simdi and not onceki_basili), simdi
    except Exception:
        return False, False

def gamepad_ates_basili(j):
    """Ateş için: SADECE sağ tetik (RT/R2, eksen 5) ya da X/Square (buton 2).
    Sağ stick sadece nişan almak için kullanılır, kendisi ateş etmez."""
    if j is None: return False
    try:
        if j.get_numaxes() > 5 and j.get_axis(5) > 0.3:
            return True
        return bool(j.get_button(2))
    except Exception:
        return False

def gamepad_nisan_yonu(j):
    """Sağ stick'ten (dx, dy) yön vektörü döndürür (aim için); stick boştaysa None."""
    if j is None: return None
    try:
        if j.get_numaxes() > 3:
            dx, dy = j.get_axis(2), j.get_axis(3)
            if abs(dx) > GAMEPAD_OLU_BOLGE or abs(dy) > GAMEPAD_OLU_BOLGE:
                return dx, dy
    except Exception:
        pass
    return None

def nisan_acisi(merkez_x, merkez_y, j=None, kam_x=0, ham_fare=False, hedef=None):
    """Nişan açısını hesaplar. j (gamepad) verilmişse artık sağ stick KULLANILMIYOR —
    bunun yerine 'hedef' (x, y) verilmişse oraya otomatik nişan alınır (en yakın
    düşman/rakip). j verilmemişse (klavye/fare oyuncusu) her zaman fare kullanılır.
    ham_fare=True: fare_konumu() yerine fare_konumu_ham() kullanır — yerel 2 kişilik
    modda klavye oyuncusunun nişanının diğer oyuncunun gamepad'inden etkilenmemesi için."""
    if j is not None and hedef is not None:
        return math.atan2(hedef[1] - merkez_y, hedef[0] - merkez_x)
    fx, fy = fare_konumu_ham() if ham_fare else fare_konumu()
    return math.atan2(fy - merkez_y, (fx + kam_x) - merkez_x)

def en_yakin_hedef_bul(merkez_x, merkez_y, dusmanlar=None, boss=None, rakip_rect=None):
    """Gamepad otomatik nişan için en yakın hedefin (x, y) merkezini döndürür, hiç hedef yoksa None.
    dusmanlar: kampanya/cybergrind düşman listesi (her biri 'rect' ve 'hayatta' içerir)
    boss: kampanya/cybergrind boss sözlüğü ('rect' ve 'is_dead' içerir)
    rakip_rect: düello/futbol modunda rakip oyuncunun rect'i"""
    en_yakin, en_yakin_mesafe = None, None
    if rakip_rect is not None:
        en_yakin = (rakip_rect.centerx, rakip_rect.centery)
    if dusmanlar:
        for d in dusmanlar:
            if not d.get('hayatta', True): continue
            mesafe = math.hypot(d['rect'].centerx - merkez_x, d['rect'].centery - merkez_y)
            if en_yakin_mesafe is None or mesafe < en_yakin_mesafe:
                en_yakin, en_yakin_mesafe = (d['rect'].centerx, d['rect'].centery), mesafe
    if boss is not None and boss.get('rect') is not None and not boss.get('is_dead', False):
        mesafe = math.hypot(boss['rect'].centerx - merkez_x, boss['rect'].centery - merkez_y)
        if en_yakin_mesafe is None or mesafe < en_yakin_mesafe:
            en_yakin, en_yakin_mesafe = (boss['rect'].centerx, boss['rect'].centery), mesafe
    return en_yakin

def gamepad_parry_tetiklendi(j, onceki_basili):
    """F/Parry için: Y/Triangle (buton 3) ya da sol tetik (LT/L2)."""
    if j is None: return False, False
    try:
        simdi = bool(j.get_button(3))
        if not simdi and j.get_numaxes() > 4:
            simdi = j.get_axis(4) > 0.3
        return (simdi and not onceki_basili), simdi
    except Exception:
        return False, False

def gamepad_silah_degistir_tetiklendi(j, onceki_lb, onceki_rb):
    """LB/RB (bumper, buton 4/5) ile silah değiştirme. Dönüş: (yon, yeni_lb, yeni_rb)
    yon: -1 (önceki silaha), 1 (sonraki silaha), 0 (değişiklik yok)."""
    if j is None: return 0, False, False
    try:
        lb, rb = bool(j.get_button(4)), bool(j.get_button(5))
        yon = 0
        if lb and not onceki_lb: yon = -1
        elif rb and not onceki_rb: yon = 1
        return yon, lb, rb
    except Exception:
...
try:
    ...
    V1_GORSEL = None  # dosya bulunamazsa değişken yine de tanımlı kalsın (NameError'ı önler)
...
def yenilikler_sayfasi():
    basliklar = [
        ("YEREL (AYNI PC) 2 KİŞİLİK", YESIL, [
            "Ağ bağlantısı olmadan, aynı ekranda 2 kişilik Düello/Futbol.",
            "P1: Klavye+Fare / P2: Gamepad, ya da iki gamepad birlikte kullanılabilir."
        ]),
        ("DİNAMİK KAMERA", TURKUAZ, [
            "Kamera oyuncuyu yumuşak bir Lerp ile takip ediyor; normal bölümler",
            "ekrandan daha geniş bir dünyada geçiyor."
        ]),
        ("STİL PUANI KOMBOLARI", PARLAK_KIRMIZI, [
            "Parry ve düşman öldürme ile stil puanı kazanılıyor: STYLISH ->",
            "DESTRUCTIVE -> RESPLENDENT -> ULTRAKILL rütbeleri."
        ]),
        ("SİLAH SİSTEMİ", TURKUAZ, [
            "Tabanca, Taramalı (Gabriel'i yenince) ve Pompalı (Swordsmachine'i",
            "yenince) silahlar arasında geçiş yapılabiliyor (1/2/3)."
        ]),
        ("CYBERGRIND MODU", MOR, [
            "Sonsuz hayatta kalma modu; her 5 roundda karışık bir boss gelir."
        ]),
    ]
    ...
    while True:
        ...
        ipucu = font_ui.render("(fare tekerleği ile kaydır)", True, GRI)
        ekran.blit(ipucu, (EN//2 - ipucu.get_width()//2, 50))
        ...
        toplam_yukseklik = y + 20

        kaydirma = max(0, min(kaydirma, max(0, toplam_yukseklik - (BOY - 90))))

        pygame.draw.rect(ekran, SIYAH, (0, 0, EN, 70))  # üst başlık alanını temiz tut
        baslik2 = font_buton.render("YENİLİKLER", True, PARLAK_KIRMIZI)
        ekran.blit(baslik2, (EN//2 - baslik2.get_width()//2, 20))
        ekran.blit(ipucu, (EN//2 - ipucu.get_width()//2, 50))
        pygame.draw.rect(ekran, SIYAH, (0, BOY - 40, EN, 40))  # alt düğme alanını temiz tut
...
def oyun_dongusu():
    ...
    # --- GAMEPAD DURUMU ---
    gp_ziplama_basili_onceki = False
    gp_ates_basili_onceki = False
    gp_parry_basili_onceki = False
    gp_silah_lb_onceki, gp_silah_rb_onceki = False, False
    SILAH_SIRASI = ["tabanca", "taramali", "pompali"]
    ...
    while True:
        ...
        # --- GAMEPAD: her karede okunur, klavye/fareyle birlikte çalışır ---
        gp = gamepad_al()
        gp_zipla_tetik, gp_ziplama_basili_onceki = gamepad_ziplama_tetiklendi(gp, gp_ziplama_basili_onceki)
        if gp_zipla_tetik and su_an - son_ziplama_girdisi > 0.1:
            if oyuncu_yerde: oyuncu_y_hiz, oyuncu_yerde, ziplama_hakki = -15, False, 1
            elif dj_aktif and ziplama_hakki == 1 and dj_cd == 0: oyuncu_y_hiz, ziplama_hakki, son_dj = -15, 2, su_an
            son_ziplama_girdisi = su_an

        gp_silah_yon, gp_silah_lb_onceki, gp_silah_rb_onceki = gamepad_silah_degistir_tetiklendi(gp, gp_silah_lb_onceki, gp_silah_rb_onceki)
        if gp_silah_yon != 0:
            uygun = [s for s in SILAH_SIRASI if s == "tabanca" or silah_kilitleri.get(s)]
            if secili_silah in uygun:
                idx = (uygun.index(secili_silah) + gp_silah_yon) % len(uygun)
                secili_silah = uygun[idx]

        gp_parry_tetik, gp_parry_basili_onceki = gamepad_parry_tetiklendi(gp, gp_parry_basili_onceki)
        if gp_parry_tetik:
            for dm in dusman_mermileri:
                if not dm.get('parried', False):
                    mesafe = math.hypot(dm['rect'].centerx - oyuncu.centerx, dm['rect'].centery - oyuncu.centery)
        ...
        if gamepad_ates_basili(gp) and secili_silah in ("tabanca", "pompali") and ates_cd == 0:
            aci = nisan_acisi(oyuncu.centerx, oyuncu.centery, gp, kam_x, hedef=en_yakin_hedef_bul(oyuncu.centerx, oyuncu.centery, dusmanlar, boss))
            if secili_silah == "tabanca":
                mermiler.append({'rect': pygame.Rect(oyuncu.centerx, oyuncu.centery, 10, 10), 'vx': math.cos(aci)*12, 'vy': math.sin(aci)*12, 'hasar': 10})
                shake_amount = 5
            else:
        ...
        if secili_silah == "taramali" and (pygame.mouse.get_pressed()[0] or gamepad_ates_basili(gp)) and ates_cd == 0:
            aci = nisan_acisi(oyuncu.centerx, oyuncu.centery, gp, kam_x, hedef=en_yakin_hedef_bul(oyuncu.centerx, oyuncu.centery, dusmanlar, boss))
        ...
        gp_hareket = gamepad_hareket_x(gp)
        if tus[pygame.K_a] or gp_hareket < 0: oyuncu.x -= 5; hareket_ediyor = True
        if tus[pygame.K_d] or gp_hareket > 0: oyuncu.x += 5; hareket_ediyor = True
...
def cybergrind_dongusu():
    ...
    while True:
        ...
        if gamepad_ates_basili(gp) and secili_silah in ("tabanca", "pompali") and ates_cd == 0:
            aci = nisan_acisi(oyuncu.centerx, oyuncu.centery, gp, hedef=en_yakin_hedef_bul(oyuncu.centerx, oyuncu.centery, durum["dusmanlar"], durum["boss"]))
            if secili_silah == "tabanca":
                mermiler.append({'rect': pygame.Rect(oyuncu.centerx, oyuncu.centery, 10, 10), 'vx': math.cos(aci)*12, 'vy': math.sin(aci)*12, 'hasar': 10})
                shake_amount = 5
            else:
...
MP_PORT = 5555
MP_MOD = "duello"  # "duello" veya "futbol" — oda kurarken belirlenir, katılan kişi otomatik miras alır
KULLANICI_ADI = "Oyuncu"  # Çevrimiçi ekranından değiştirilebilir, can barının üstünde gösterilir
MP_PROTOKOL_SURUMU = 2  # Relay sunucuyla uyumluluk kontrolü — sunucu ile eşleşmezse bağlanamaz
...
def baglanti_koptu_ekrani():
    while True:
        ekran.fill(SIYAH); fare = fare_konumu()
        t = font_baslik.render("Rakip Oyundan Ayrıldı!", True, PARLAK_KIRMIZI)
        ekran.blit(t, (EN//2 - t.get_width()//2, 230))

        btn_ana_menu = pygame.Rect(EN//2 - 130, 320, 260, 48)
        hov = btn_ana_menu.collidepoint(fare)
        c = SARI if hov else BEYAZ
        pygame.draw.rect(ekran, c, btn_ana_menu, 2)
        bm_yazi = font_buton.render("ANA MENÜYE DÖN", True, c)
        ekran.blit(bm_yazi, (btn_ana_menu.centerx - bm_yazi.get_width()//2, btn_ana_menu.centery - 11))

        for event in pygame.event.get():
            if event.type == pygame.QUIT: pygame.quit(); sys.exit()
            if event.type == pygame.KEYDOWN and event.key == pygame.K_ESCAPE:
                ana_menu(); return
            if event.type == pygame.MOUSEBUTTONDOWN and btn_ana_menu.collidepoint(fare):
                ana_menu(); return
        scanline_ciz(); ekrani_guncelle()

def mp_menu():
    global MP_MOD, KULLANICI_ADI
    while True:
        ekran.fill(SIYAH); L = metinler[ayarlar["dil"]]; fare = fare_konumu()
        baslik = font_buton.render("ÇEVRİMİÇİ", True, TURKUAZ)
        ekran.blit(baslik, (EN//2 - baslik.get_width()//2, 70))

        # --- Kullanıcı adı ---
        btn_isim = pygame.Rect(250, 100, 300, 34)
        hov_isim = btn_isim.collidepoint(fare)
        pygame.draw.rect(ekran, (SARI if hov_isim else BEYAZ), btn_isim, 2)
        isim_yazi = font_buton.render(f"İSİM: {KULLANICI_ADI}", True, (SARI if hov_isim else BEYAZ))
        ekran.blit(isim_yazi, (btn_isim.centerx - isim_yazi.get_width()//2, btn_isim.centery - 11))

        aciklamalar = [
            "AYNI AĞ: Aynı Wi-Fi'daki biriyle. Biri SUNUCU KUR, diğeri KATIL (IP ile).",
            "İNTERNET: Uzaktaki biriyle. Oda kurup arkadaşınla paylaş, ya da açık odalara katıl."
        ]
        y = 142
        for s in aciklamalar:
            t = font_ui.render(s, True, GRI)
            ekran.blit(t, (EN//2 - t.get_width()//2, y)); y += 20

        # --- LAN bölümü ---
        lan_lbl = font_ui.render("— AYNI AĞ (LAN) —", True, GRI)
        ekran.blit(lan_lbl, (EN//2 - lan_lbl.get_width()//2, 190))

        btn_mod = pygame.Rect(300, 210, 200, 32)
        mod_yazi = "MOD: DÜELLO" if MP_MOD == "duello" else "MOD: FUTBOL ⚽"
        mod_renk = SARI if btn_mod.collidepoint(fare) else (TURKUAZ if MP_MOD == "duello" else YESIL)
        pygame.draw.rect(ekran, mod_renk, btn_mod, 2)
        mt = font_buton.render(mod_yazi, True, mod_renk)
        ekran.blit(mt, (btn_mod.centerx - mt.get_width()//2, btn_mod.centery - 10))

        btn_host = pygame.Rect(180, 250, 205, 40)
        btn_join = pygame.Rect(415, 250, 205, 40)

        # --- İnternet bölümü ---
        net_lbl = font_ui.render("— İNTERNET (LOBİ) —", True, MOR)
        ekran.blit(net_lbl, (EN//2 - net_lbl.get_width()//2, 305))

        btn_oda_kur = pygame.Rect(180, 325, 205, 44)
        btn_oda_ara = pygame.Rect(415, 325, 205, 44)

        # --- Yerel (aynı PC) bölümü ---
        yerel_lbl = font_ui.render("— YEREL (AYNI PC, 2 KİŞİLİK) —", True, YESIL)
        ekran.blit(yerel_lbl, (EN//2 - yerel_lbl.get_width()//2, 385))
        btn_yerel = pygame.Rect(300, 405, 200, 40)

        btn_geri = pygame.Rect(300, 470, 200, 44)

        for r, t, renk in [(btn_host, "SUNUCU KUR", TURKUAZ), (btn_join, "KATIL (IP)", TURKUAZ),
                            (btn_oda_kur, "ODA OLUŞTUR", MOR), (btn_oda_ara, "ODALARI ARA", MOR),
                            (btn_yerel, "YEREL BAŞLAT", YESIL), (btn_geri, L["geri"], GRI)]:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN and event.key == pygame.K_ESCAPE: return
            if event.type == pygame.MOUSEBUTTONDOWN:
                if btn_isim.collidepoint(fare):
                    yeni_isim = metin_girisi("Kullanıcı Adın:", KULLANICI_ADI)
                    if yeni_isim and yeni_isim.strip():
                        KULLANICI_ADI = yeni_isim.strip()[:16]
                if btn_mod.collidepoint(fare):
                    MP_MOD = "futbol" if MP_MOD == "duello" else "duello"
                if btn_host.collidepoint(fare): mp_sunucu_kur()
                if btn_join.collidepoint(fare): mp_istemci_baglan()
                if btn_oda_kur.collidepoint(fare): mp_oda_olustur_ekrani()
                if btn_oda_ara.collidepoint(fare): mp_odalari_ara_ekrani()
                if btn_yerel.collidepoint(fare): yerel_mp_ayar_ekrani()
                if btn_geri.collidepoint(fare): return
        scanline_ciz(); ekrani_guncelle()
...
def _relay_ham_baglan():
    """Relay sunucuya TLS+WebSocket bağlantısı kurar (senkron, arka plan thread'i içinde
    çağrılmalı). Başarılı olursa mesaj gönderip/alabilen bir soket-benzeri nesne döndürür.
    10 saniyelik zaman aşımı SADECE bağlanma anı için — bağlantı kurulduktan sonra
    (örn. rakip katılmasını beklerken) süresiz bekleyebilsin diye kaldırılıyor."""
    ham_soket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    ham_soket.settimeout(10)
    ham_soket.connect((RELAY_SUNUCU_ADRES, RELAY_SUNUCU_PORT))
    if RELAY_SUNUCU_PORT == 443:
        baglam = ssl.create_default_context()
        tls_soket = baglam.wrap_socket(ham_soket, server_hostname=RELAY_SUNUCU_ADRES)
        _ws_el_sikismasi(tls_soket, RELAY_SUNUCU_ADRES)
        tls_soket.settimeout(None)  # bağlantı kuruldu, artık zaman aşımı olmasın
        return _WebSocketSoket(tls_soket)
    ham_soket.settimeout(None)
    return ham_soket

def _mp_oda_kur_baglan(oda_adi, sifre, mod):
    """'ODA OLUŞTUR' onaylanınca çağrılır: sunucuya bağlanır, odayı kurar, rakip bekler."""
    global MP_MOD
    kutu_b = {"durum": "baglaniyor", "is_host": None, "hata": None, "soket": None, "rakip_adi": "RAKİP"}

    def calis():
        try:
            soket = _relay_ham_baglan()
            kutu_b["soket"] = soket
            soket.sendall((json.dumps({"t": "oda_kur", "oda_adi": oda_adi, "sifre": sifre,
                                        "mod": mod, "kullanici_adi": KULLANICI_ADI,
                                        "surum": MP_PROTOKOL_SURUMU}) + "\n").encode())
            dosya = soket.makefile('r')
            for satir in dosya:
                try: veri = json.loads(satir.strip())
                except Exception: continue
                sm = veri.get("sys")
                if sm == "bekliyor": kutu_b["durum"] = "bekliyor"
                elif sm == "surum_uyumsuz":
                    kutu_b["hata"] = (f"Oyun sürümün güncel değil (senin: {veri.get('sizin_surumu','?')}, "
                                       f"gereken: {veri.get('sunucu_surumu','?')}). Lütfen en güncel "
                                       f"ULTRAPARKOUR.py dosyasını indirip tekrar dene.")
                    return
                elif sm == "oda_var": kutu_b["hata"] = "Bu isimde bir oda zaten var, başka bir isim dene."; return
                elif sm == "hata": kutu_b["hata"] = veri.get("mesaj", "Bilinmeyen hata"); return
                elif sm == "hazir":
                    kutu_b["is_host"] = veri.get("host", True)
                    kutu_b["rakip_adi"] = veri.get("rakip_adi", "RAKİP")
                    kutu_b["durum"] = "hazir"
                    return
        except Exception as e:
            kutu_b["hata"] = str(e)
    ...
    while True:
        ekran.fill(SIYAH)
        b = font_buton.render(f"ODA: {oda_adi}", True, SARI)
        ekran.blit(b, (EN//2 - b.get_width()//2, 180))
        durum = kutu_b["durum"]
        mesaj = {"baglaniyor": "Sunucuya bağlanılıyor...",
                 "bekliyor": "Arkadaşının odana katılması bekleniyor..."}.get(durum, "...")
        m = font_ui.render(mesaj, True, TURKUAZ)
        ekran.blit(m, (EN//2 - m.get_width()//2, 240))
        ekran.blit(font_ui.render("ESC: İptal", True, GRI), (EN//2 - 40, 290))

        for event in pygame.event.get():
            if event.type == pygame.QUIT: pygame.quit(); sys.exit()
            if event.type == pygame.KEYDOWN and event.key == pygame.K_ESCAPE:
                try:
                    if kutu_b["soket"]: kutu_b["soket"].close()
                except Exception: pass
                return

        if kutu_b["hata"]:
            hata_goster(kutu_b["hata"]); return
        if kutu_b["durum"] == "hazir":
            soket = kutu_b["soket"]; soket.settimeout(None)
            MP_MOD = mod
            mp_dongusu_baslat(soket, kutu_b["is_host"], kutu_b["rakip_adi"])
            return
        scanline_ciz(); ekrani_guncelle(); saat.tick(30)

def mp_oda_olustur_ekrani():
    oda_adi_deger = [""]
    sifre_deger = [""]
    aktif_alan = ["oda"]
    mod_secim = [MP_MOD]
    uyari_metni = [""]
    uyari_bitis = [0]
    sifre_uyarisi_onaylandi = [False]

    while True:
        ekran.fill(SIYAH); fare = fare_konumu()
        baslik = font_buton.render("LOBİ OLUŞTUR", True, MOR)
        ekran.blit(baslik, (EN//2 - baslik.get_width()//2, 55))

        kutu_oda = pygame.Rect(EN//2 - 160, 115, 320, 40)
        renk_oda = SARI if aktif_alan[0] == "oda" else BEYAZ
        pygame.draw.rect(ekran, renk_oda, kutu_oda, 2)
        ekran.blit(font_ui.render("ODA ADI:", True, GRI), (kutu_oda.x, kutu_oda.y - 20))
        imlec1 = "_" if (int(time.time()*2) % 2 == 0 and aktif_alan[0] == "oda") else ""
        ekran.blit(font_buton.render(oda_adi_deger[0] + imlec1, True, BEYAZ), (kutu_oda.x + 10, kutu_oda.y + 8))

        kutu_sifre = pygame.Rect(EN//2 - 160, 185, 320, 40)
        renk_sifre = SARI if aktif_alan[0] == "sifre" else BEYAZ
        pygame.draw.rect(ekran, renk_sifre, kutu_sifre, 2)
        ekran.blit(font_ui.render("ŞİFRE (isteğe bağlı):", True, GRI), (kutu_sifre.x, kutu_sifre.y - 20))
        imlec2 = "_" if (int(time.time()*2) % 2 == 0 and aktif_alan[0] == "sifre") else ""
        ekran.blit(font_buton.render("*" * len(sifre_deger[0]) + imlec2, True, BEYAZ), (kutu_sifre.x + 10, kutu_sifre.y + 8))

        # Kırmızı daire içinde ünlem — üstüne gelince açıklama çıkar
        ikon_merkez = (kutu_sifre.centerx, kutu_sifre.bottom + 22)
        pygame.draw.circle(ekran, PARLAK_KIRMIZI, ikon_merkez, 12, 2)
        unlem = font_buton.render("!", True, PARLAK_KIRMIZI)
        ekran.blit(unlem, (ikon_merkez[0] - unlem.get_width()//2, ikon_merkez[1] - unlem.get_height()//2))
        fare_ikon_ustunde = math.hypot(fare[0] - ikon_merkez[0], fare[1] - ikon_merkez[1]) <= 14
        if fare_ikon_ustunde:
            ip = font_ui.render("Şifre kutucuğu boş bırakılırsa isteyen herkes katılabilir.", True, SARI)
            ekran.blit(ip, (EN//2 - ip.get_width()//2, ikon_merkez[1] + 16))

        btn_mod = pygame.Rect(EN//2 - 100, 275, 200, 34)
        mod_yazi = "MOD: DÜELLO" if mod_secim[0] == "duello" else "MOD: FUTBOL ⚽"
        mod_renk = SARI if btn_mod.collidepoint(fare) else (TURKUAZ if mod_secim[0] == "duello" else YESIL)
        pygame.draw.rect(ekran, mod_renk, btn_mod, 2)
        mt = font_buton.render(mod_yazi, True, mod_renk)
        ekran.blit(mt, (btn_mod.centerx - mt.get_width()//2, btn_mod.centery - 10))

        if time.time() < uyari_bitis[0]:
            for i, satir in enumerate(uyari_metni[0].split("\n")):
                uy = font_ui.render(satir, True, PARLAK_KIRMIZI)
                ekran.blit(uy, (EN//2 - uy.get_width()//2, 315 + i*18))

        btn_olustur = pygame.Rect(EN//2 - 100, 355, 200, 44)
        hov_ol = btn_olustur.collidepoint(fare)
        pygame.draw.rect(ekran, (SARI if hov_ol else BEYAZ), btn_olustur, 2)
        ol_yazi = font_buton.render("OLUŞTUR", True, (SARI if hov_ol else BEYAZ))
        ekran.blit(ol_yazi, (btn_olustur.centerx - ol_yazi.get_width()//2, btn_olustur.centery - 11))
        ekran.blit(font_ui.render("ESC: Geri", True, GRI), (EN//2 - 40, 410))

        for event in pygame.event.get():
            if event.type == pygame.QUIT: pygame.quit(); sys.exit()
            if event.type == pygame.KEYDOWN and event.key == pygame.K_ESCAPE: return
            if event.type == pygame.MOUSEBUTTONDOWN:
                if kutu_oda.collidepoint(fare): aktif_alan[0] = "oda"
                elif kutu_sifre.collidepoint(fare): aktif_alan[0] = "sifre"
                elif btn_mod.collidepoint(fare):
                    mod_secim[0] = "futbol" if mod_secim[0] == "duello" else "duello"
                elif btn_olustur.collidepoint(fare):
                    if not oda_adi_deger[0].strip():
                        uyari_metni[0] = "Oda adı boş olamaz!"
                        uyari_bitis[0] = time.time() + 2.5
                    elif not sifre_deger[0] and not sifre_uyarisi_onaylandi[0]:
                        uyari_metni[0] = "Şifre kutucuğu boş bırakılırsa isteyen herkes katılabilir.\nDevam etmek için tekrar OLUŞTUR'a bas."
                        uyari_bitis[0] = time.time() + 4
                        sifre_uyarisi_onaylandi[0] = True
                    else:
                        _mp_oda_kur_baglan(oda_adi_deger[0].strip(), sifre_deger[0], mod_secim[0])
                        return
            if event.type == pygame.KEYDOWN:
                hedef = oda_adi_deger if aktif_alan[0] == "oda" else sifre_deger
                if event.key == pygame.K_BACKSPACE:
                    hedef[0] = hedef[0][:-1]
                elif event.key == pygame.K_TAB:
                    aktif_alan[0] = "sifre" if aktif_alan[0] == "oda" else "oda"
                elif event.unicode and event.unicode.isprintable() and len(hedef[0]) < 24:
                    hedef[0] += event.unicode
                    sifre_uyarisi_onaylandi[0] = False

        scanline_ciz(); ekrani_guncelle(); saat.tick(30)

def _mp_oda_katil_baglan(oda_adi, sifre):
    """Listeden bir odaya katılma onaylanınca çağrılır."""
    global MP_MOD
    kutu_b = {"durum": "baglaniyor", "is_host": None, "hata": None, "soket": None, "mod": "duello", "rakip_adi": "RAKİP"}

    def calis():
        try:
            soket = _relay_ham_baglan()
            kutu_b["soket"] = soket
            soket.sendall((json.dumps({"t": "oda_katil", "oda_adi": oda_adi, "sifre": sifre,
                                        "kullanici_adi": KULLANICI_ADI, "surum": MP_PROTOKOL_SURUMU}) + "\n").encode())
            dosya = soket.makefile('r')
            for satir in dosya:
                try: veri = json.loads(satir.strip())
                except Exception: continue
                ...
                if sm == "surum_uyumsuz":
                    kutu_b["hata"] = (f"Oyun sürümün güncel değil (senin: {veri.get('sizin_surumu','?')}, "
                                       ...
                                       f"ULTRAPARKOUR.py dosyasını indirip tekrar dene.")
                    return
                elif sm == "oda_yok": kutu_b["hata"] = "Bu oda artık yok (kapanmış olabilir)."; return
                elif sm == "doldu": kutu_b["hata"] = "Bu oda dolu (2 kişi)."; return
                elif sm == "sifre_yanlis": kutu_b["hata"] = "Şifre yanlış."; return
                elif sm == "hazir":
                    kutu_b["is_host"] = veri.get("host", False)
                    kutu_b["mod"] = veri.get("mod", "duello")
                    kutu_b["rakip_adi"] = veri.get("rakip_adi", "RAKİP")
                    kutu_b["durum"] = "hazir"
                    return
        except Exception as e:
    ...
    while True:
        ekran.fill(SIYAH)
        b = font_buton.render(f"{oda_adi} odasına bağlanılıyor...", True, TURKUAZ)
        ekran.blit(b, (EN//2 - b.get_width()//2, 220))
        ekran.blit(font_ui.render("ESC: İptal", True, GRI), (EN//2 - 40, 280))
...
def mp_odalari_ara_ekrani():
    kutu_b = {"durum": "baglaniyor", "hata": None, "soket": None, "odalar": []}
    ODA_SAYFA_BOYU = 6
    sayfa = [0]
    ...
    def calis():
        try:
            soket = _relay_ham_baglan()
            kutu_b["soket"] = soket
            soket.sendall((json.dumps({"t": "oda_listesi_iste", "surum": MP_PROTOKOL_SURUMU}) + "\n").encode())
            dosya = soket.makefile('r')
            for satir in dosya:
                try: veri = json.loads(satir.strip())
                except Exception: continue
                if veri.get("sys") == "oda_listesi":
                    kutu_b["odalar"] = veri.get("odalar", [])
                    kutu_b["durum"] = "listelendi"
                    return
        except Exception as e:
            kutu_b["hata"] = str(e)
    ...
    while True:
        ekran.fill(SIYAH); fare = fare_konumu()
        baslik = font_buton.render("ODALARI ARA", True, MOR)
        ekran.blit(baslik, (EN//2 - baslik.get_width()//2, 40))

        if kutu_b["hata"]:
            hata_goster(kutu_b["hata"]); return

        toplam_sayfa = 1
        if kutu_b["durum"] == "baglaniyor":
            m = font_ui.render("Oda listesi alınıyor...", True, TURKUAZ)
            ekran.blit(m, (EN//2 - m.get_width()//2, 200))
        else:
            odalar = kutu_b["odalar"]
            toplam_sayfa = max(1, (len(odalar) + ODA_SAYFA_BOYU - 1) // ODA_SAYFA_BOYU)
            sayfa[0] = max(0, min(sayfa[0], toplam_sayfa - 1))
            if not odalar:
                m = font_ui.render("Şu an açık oda yok. Bir tane sen kur!", True, GRI)
                ekran.blit(m, (EN//2 - m.get_width()//2, 200))
            baslangic = sayfa[0] * ODA_SAYFA_BOYU
            gorunen_odalar = odalar[baslangic: baslangic + ODA_SAYFA_BOYU]
            y = 90
            for oda in gorunen_odalar:
                r = pygame.Rect(EN//2 - 260, y, 520, 44)
                hov = r.collidepoint(fare)
                pygame.draw.rect(ekran, (SARI if hov else BEYAZ), r, 2)
                kilit = "🔒" if oda.get("sifreli") else ""
                mod_kisa = "DÜELLO" if oda.get("mod") == "duello" else "FUTBOL"
                yazi = f"{oda.get('ad','?')}  [{mod_kisa}]  Host: {oda.get('host','?')} {kilit}"
                t = font_ui.render(yazi, True, (SARI if hov else BEYAZ))
                ekran.blit(t, (r.x + 12, r.centery - 8))
                ekran.blit(font_buton.render("KATIL", True, TURKUAZ), (r.right - 90, r.centery - 11))
                oda['_rect'] = r
                y += 52

            if toplam_sayfa > 1:
                sayfa_yazi = font_ui.render(f"Sayfa {sayfa[0]+1}/{toplam_sayfa}", True, GRI)
                ekran.blit(sayfa_yazi, (EN//2 - sayfa_yazi.get_width()//2, y + 6))

        # --- Sayfalama düğmeleri (sol/sağ) — bir sayfadan fazla oda varsa görünür ---
        btn_sol = pygame.Rect(EN//2 - 320, 90, 44, 300)
        btn_sag = pygame.Rect(EN//2 + 276, 90, 44, 300)
        if toplam_sayfa > 1:
            if sayfa[0] > 0:
                hov = btn_sol.collidepoint(fare); c = SARI if hov else BEYAZ
                pygame.draw.rect(ekran, c, btn_sol, 2)
                ok = font_baslik.render("◄", True, c)
                ekran.blit(ok, (btn_sol.centerx - ok.get_width()//2, btn_sol.centery - ok.get_height()//2))
            if sayfa[0] < toplam_sayfa - 1:
                hov = btn_sag.collidepoint(fare); c = SARI if hov else BEYAZ
                pygame.draw.rect(ekran, c, btn_sag, 2)
                ok = font_baslik.render("►", True, c)
                ekran.blit(ok, (btn_sag.centerx - ok.get_width()//2, btn_sag.centery - ok.get_height()//2))

        btn_yenile = pygame.Rect(EN//2 - 210, BOY - 90, 180, 40)
        btn_geri = pygame.Rect(EN//2 + 30, BOY - 90, 180, 40)
        for r, t in [(btn_yenile, "YENİLE"), (btn_geri, "GERİ")]:
            hov = r.collidepoint(fare); c = SARI if hov else BEYAZ
            pygame.draw.rect(ekran, c, r, 2)
            ts = font_buton.render(t, True, c)
            ekran.blit(ts, (r.centerx - ts.get_width()//2, r.centery - 11))
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN and event.key == pygame.K_ESCAPE:
                ...
                return
            if event.type == pygame.MOUSEBUTTONDOWN:
                if toplam_sayfa > 1 and sayfa[0] > 0 and btn_sol.collidepoint(fare):
                    sayfa[0] -= 1; continue
                if toplam_sayfa > 1 and sayfa[0] < toplam_sayfa - 1 and btn_sag.collidepoint(fare):
                    sayfa[0] += 1; continue
                if btn_geri.collidepoint(fare):
                    try:
                        if kutu_b["soket"]: kutu_b["soket"].close()
                    except Exception: pass
                    return
                if btn_yenile.collidepoint(fare):
                    try:
                        if kutu_b["soket"]: kutu_b["soket"].close()
                    except Exception: pass
                    return mp_odalari_ara_ekrani()
                if kutu_b["durum"] == "listelendi":
                    _baslangic = sayfa[0] * ODA_SAYFA_BOYU
                    for oda in kutu_b["odalar"][_baslangic: _baslangic + ODA_SAYFA_BOYU]:
                        if oda.get('_rect') and oda['_rect'].collidepoint(fare):
                            try:
                                if kutu_b["soket"]: kutu_b["soket"].close()
                            except Exception: pass
                            sifre = ""
                            if oda.get("sifreli"):
                                sifre = metin_girisi(f"'{oda.get('ad')}' odası şifreli — Şifreyi gir:", "")
                                if sifre is None: return
                            _mp_oda_katil_baglan(oda.get("ad"), sifre or "")
                            return

        scanline_ciz(); ekrani_guncelle(); saat.tick(30)

def yerel_mp_ayar_ekrani():
    """Yerel (aynı PC) 2 kişilik mod öncesi ayar ekranı: DÜELLO/FUTBOL ve
    KLAVYE+GAMEPAD / GAMEPAD+GAMEPAD seçimi yapılır."""
    mod = ["duello"]
    sema = ["kb_gp"]
    while True:
        ekran.fill(SIYAH); fare = fare_konumu()
        baslik = font_buton.render("YEREL 2 KİŞİLİK", True, YESIL)
        ekran.blit(baslik, (EN//2 - baslik.get_width()//2, 60))

        btn_mod = pygame.Rect(EN//2 - 130, 140, 260, 44)
        mod_yazi = "MOD: DÜELLO" if mod[0] == "duello" else "MOD: FUTBOL ⚽"
        mod_renk = SARI if btn_mod.collidepoint(fare) else (TURKUAZ if mod[0] == "duello" else MOR)
        pygame.draw.rect(ekran, mod_renk, btn_mod, 2)
        mt = font_buton.render(mod_yazi, True, mod_renk)
        ekran.blit(mt, (btn_mod.centerx - mt.get_width()//2, btn_mod.centery - 11))

        btn_sema = pygame.Rect(EN//2 - 160, 210, 320, 44)
        sema_yazi = "P1: KLAVYE+FARE   P2: GAMEPAD" if sema[0] == "kb_gp" else "P1: GAMEPAD 1   P2: GAMEPAD 2"
        sema_renk = SARI if btn_sema.collidepoint(fare) else BEYAZ
        pygame.draw.rect(ekran, sema_renk, btn_sema, 2)
        st = font_ui.render(sema_yazi, True, sema_renk)
        ekran.blit(st, (btn_sema.centerx - st.get_width()//2, btn_sema.centery - 8))

        gp_sayisi = pygame.joystick.get_count()
        uyari = None
        if sema[0] == "gp_gp" and gp_sayisi < 2:
            uyari = f"En az 2 gamepad gerekli (şu an takılı: {gp_sayisi})"
        elif sema[0] == "kb_gp" and gp_sayisi < 1:
            uyari = "P2 için en az 1 gamepad gerekli (hiç takılı değil)"
        if uyari:
            uy = font_ui.render(uyari, True, PARLAK_KIRMIZI)
            ekran.blit(uy, (EN//2 - uy.get_width()//2, 265))

        aciklamalar = [
            "P1 (klavye): A/D hareket, W zıpla, F parry, 1/2/3 silah, fare nişan+sol tık ateş",
            "Gamepad: sol stick hareket, A zıpla, sağ stick nişan, RT ateş, LB/RB silah, Y parry",
        ]
        y = 310
        for s in aciklamalar:
            t = font_ui.render(s, True, GRI)
        ...
        btn_baslat = pygame.Rect(EN//2 - 110, 400, 220, 48)
        baslat_aktif = not uyari
        b_renk = (SARI if btn_baslat.collidepoint(fare) else YESIL) if baslat_aktif else GRI
        pygame.draw.rect(ekran, b_renk, btn_baslat, 2)
        bt = font_buton.render("BAŞLAT", True, b_renk)
        ekran.blit(bt, (btn_baslat.centerx - bt.get_width()//2, btn_baslat.centery - 11))

        btn_geri = pygame.Rect(EN//2 - 100, 470, 200, 40)
        hov_geri = btn_geri.collidepoint(fare); c_geri = SARI if hov_geri else GRI
        pygame.draw.rect(ekran, c_geri, btn_geri, 2)
        gt = font_buton.render("GERİ", True, c_geri)
        ekran.blit(gt, (btn_geri.centerx - gt.get_width()//2, btn_geri.centery - 11))
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if btn_mod.collidepoint(fare):
                    mod[0] = "futbol" if mod[0] == "duello" else "duello"
                if btn_sema.collidepoint(fare):
                    sema[0] = "gp_gp" if sema[0] == "kb_gp" else "kb_gp"
                if btn_geri.collidepoint(fare):
                    return
                if baslat_aktif and btn_baslat.collidepoint(fare):
                    if mod[0] == "duello":
                        yerel_duello_dongusu(sema[0])
                    else:
                        yerel_futbol_dongusu(sema[0])
                    return
        scanline_ciz(); ekrani_guncelle()

def yerel_duello_dongusu(sema):
    """Aynı bilgisayarda, tek pencerede 2 kişilik düello. Ağ bağlantısı YOK — her iki
    oyuncu da aynı süreçte, aynı ekranda simüle ediliyor.
    sema: 'kb_gp' -> P1 klavye+fare, P2 gamepad(0) | 'gp_gp' -> P1 gamepad(0), P2 gamepad(1)."""
    global shake_amount
    p1_gp_idx = None if sema == "kb_gp" else 0
    p2_gp_idx = 0 if sema == "kb_gp" else 1

    zemin = pygame.Rect(0, 550, EN, 50)
    platformlar = [zemin, pygame.Rect(150, 400, 180, 20), pygame.Rect(470, 400, 180, 20), pygame.Rect(325, 280, 150, 20)]

    def yeni_oyuncu_durumu(x):
        return {
            'rect': pygame.Rect(x, 500, 30, 30), 'y_hiz': 0, 'yerde': False,
            'son_ziplama': 0, 'can': 100, 'silah': "tabanca", 'son_ates': 0,
            'mermiler': [], 'gp_zipla_onceki': False, 'gp_parry_onceki': False,
            'gp_lb_onceki': False, 'gp_rb_onceki': False,
        }

    p1 = yeni_oyuncu_durumu(100)
    p2 = yeni_oyuncu_durumu(EN - 130)
    silah_beklemeleri = {"tabanca": 3, "taramali": 0.12, "pompali": 1.1}
    SILAH_SIRASI = ["tabanca", "taramali", "pompali"]
    parry_radius = 65
    kazandi = None  # None: devam, 1: P1 kazandı, 2: P2 kazandı

    muzik_cal("oyun1.mp3")

    def ates_et(o, aci, karsi_taraf_adi):
        cd_kalan = max(0, silah_beklemeleri.get(o['silah'], 3) - (time.time() - o['son_ates']))
        if cd_kalan > 0: return
        if o['silah'] == "tabanca":
            o['mermiler'].append({'rect': pygame.Rect(o['rect'].centerx, o['rect'].centery, 10, 10),
                                   'vx': math.cos(aci)*12, 'vy': math.sin(aci)*12, 'hasar': 10})
            shake_amount = 5
        elif o['silah'] == "pompali":
            for sapma in (-0.28, -0.14, 0, 0.14, 0.28):
                a2 = aci + sapma
                o['mermiler'].append({'rect': pygame.Rect(o['rect'].centerx, o['rect'].centery, 8, 8),
                                       'vx': math.cos(a2)*11, 'vy': math.sin(a2)*11, 'hasar': 6})
            shake_amount = 14
        else:  # taramali
            o['mermiler'].append({'rect': pygame.Rect(o['rect'].centerx, o['rect'].centery, 8, 8),
                                   'vx': math.cos(aci)*13, 'vy': math.sin(aci)*13, 'hasar': 4})
            shake_amount = 3
        o['son_ates'] = time.time()
        if ses_atis: ses_atis.play()
        parcacik_ekle(o['rect'].centerx, o['rect'].centery, TURKUAZ, 5)

    while True:
        su_an = time.time()
        gp1 = gamepad_al(p1_gp_idx) if p1_gp_idx is not None else None
        gp2 = gamepad_al(p2_gp_idx)

        for event in pygame.event.get():
            if event.type == pygame.QUIT: pygame.quit(); sys.exit()
            if event.type == pygame.KEYDOWN and event.key == pygame.K_F11: tam_ekran_ac_kapa()
            if event.type == pygame.KEYDOWN and event.key == pygame.K_ESCAPE: return
            if kazandi is not None and event.type == pygame.KEYDOWN and event.key == pygame.K_r:
                p1 = yeni_oyuncu_durumu(100); p2 = yeni_oyuncu_durumu(EN - 130); kazandi = None
            if kazandi is None and sema == "kb_gp" and event.type == pygame.KEYDOWN:
                if event.key == pygame.K_w and su_an - p1['son_ziplama'] > 0.1:
                    if p1['yerde']: p1['y_hiz'], p1['yerde'] = -15, False
                    p1['son_ziplama'] = su_an
                if event.key == pygame.K_1: p1['silah'] = "tabanca"
                if event.key == pygame.K_2: p1['silah'] = "taramali"
                if event.key == pygame.K_3: p1['silah'] = "pompali"
                if event.key == pygame.K_f:
                    for dm in p2['mermiler'][:]:
                        mesafe = math.hypot(dm['rect'].centerx - p1['rect'].centerx, dm['rect'].centery - p1['rect'].centery)
                        if mesafe <= parry_radius:
                            p2['mermiler'].remove(dm)
                            dm['vx'] *= -2.2; dm['vy'] *= -2.2; dm['hasar'] = 15
                            p1['mermiler'].append(dm)
                            shake_amount = 15
                            parcacik_ekle(dm['rect'].centerx, dm['rect'].centery, SARI, 20)
                            if ses_atis: ses_atis.play()
                            break
            if kazandi is None and sema == "kb_gp" and event.type == pygame.MOUSEBUTTONDOWN and event.button == 1 and p1['silah'] in ("tabanca", "pompali"):
                aci = nisan_acisi(p1['rect'].centerx, p1['rect'].centery, None, 0, ham_fare=True)
                ates_et(p1, aci, "P2")
        ...
        if kazandi is None:
            ...
            if sema == "kb_gp":
                tus = pygame.key.get_pressed()
                if tus[pygame.K_a]: p1['rect'].x -= 5
                if tus[pygame.K_d]: p1['rect'].x += 5
            else:  # gp_gp: P1 de gamepad
                zip_tetik, p1['gp_zipla_onceki'] = gamepad_ziplama_tetiklendi(gp1, p1['gp_zipla_onceki'])
                if zip_tetik and su_an - p1['son_ziplama'] > 0.1:
                    if p1['yerde']: p1['y_hiz'], p1['yerde'] = -15, False
                    p1['son_ziplama'] = su_an
                silah_yon, p1['gp_lb_onceki'], p1['gp_rb_onceki'] = gamepad_silah_degistir_tetiklendi(gp1, p1['gp_lb_onceki'], p1['gp_rb_onceki'])
                if silah_yon != 0:
                    idx = (SILAH_SIRASI.index(p1['silah']) + silah_yon) % len(SILAH_SIRASI)
                    p1['silah'] = SILAH_SIRASI[idx]
                parry_tetik, p1['gp_parry_onceki'] = gamepad_parry_tetiklendi(gp1, p1['gp_parry_onceki'])
                if parry_tetik:
                    for dm in p2['mermiler'][:]:
                        mesafe = math.hypot(dm['rect'].centerx - p1['rect'].centerx, dm['rect'].centery - p1['rect'].centery)
                        if mesafe <= parry_radius:
                            ...
                            if ses_atis: ses_atis.play()
                            break
                cd_kalan = max(0, silah_beklemeleri.get(p1['silah'], 3) - (su_an - p1['son_ates']))
                if gamepad_ates_basili(gp1) and p1['silah'] in ("tabanca", "pompali") and cd_kalan == 0:
                    aci = nisan_acisi(p1['rect'].centerx, p1['rect'].centery, gp1, hedef=(p2['rect'].centerx, p2['rect'].centery))
                    ates_et(p1, aci, "P2")
                gp_hareket1 = gamepad_hareket_x(gp1)
                if gp_hareket1 < 0: p1['rect'].x -= 5
                if gp_hareket1 > 0: p1['rect'].x += 5

            if p1['silah'] == "taramali":
                cd_kalan = max(0, silah_beklemeleri.get(p1['silah'], 3) - (su_an - p1['son_ates']))
                basili = (sema == "kb_gp" and pygame.mouse.get_pressed()[0]) or gamepad_ates_basili(gp1)
                if basili and cd_kalan == 0:
                    aci = nisan_acisi(p1['rect'].centerx, p1['rect'].centery, gp1 if sema != "kb_gp" else None, 0, ham_fare=(sema == "kb_gp"), hedef=(p2['rect'].centerx, p2['rect'].centery))
                    ates_et(p1, aci, "P2")

            p1['rect'].x = max(0, min(EN - 30, p1['rect'].x))
            p1['y_hiz'] += 0.8; p1['rect'].y += p1['y_hiz']
            p1['yerde'] = False
            for p in platformlar:
                if p1['rect'].colliderect(p) and p1['y_hiz'] > 0:
                    p1['rect'].bottom, p1['y_hiz'], p1['yerde'] = p.top, 0, True
            if p1['rect'].y > BOY + 100: p1['rect'].x, p1['rect'].y, p1['y_hiz'] = 100, 500, 0

            # --- P2 kontrolü (her zaman gamepad) ---
            zip_tetik2, p2['gp_zipla_onceki'] = gamepad_ziplama_tetiklendi(gp2, p2['gp_zipla_onceki'])
            if zip_tetik2 and su_an - p2['son_ziplama'] > 0.1:
                if p2['yerde']: p2['y_hiz'], p2['yerde'] = -15, False
                p2['son_ziplama'] = su_an
            silah_yon2, p2['gp_lb_onceki'], p2['gp_rb_onceki'] = gamepad_silah_degistir_tetiklendi(gp2, p2['gp_lb_onceki'], p2['gp_rb_onceki'])
            if silah_yon2 != 0:
                idx = (SILAH_SIRASI.index(p2['silah']) + silah_yon2) % len(SILAH_SIRASI)
                p2['silah'] = SILAH_SIRASI[idx]
            parry_tetik2, p2['gp_parry_onceki'] = gamepad_parry_tetiklendi(gp2, p2['gp_parry_onceki'])
            if parry_tetik2:
                for dm in p1['mermiler'][:]:
                    mesafe = math.hypot(dm['rect'].centerx - p2['rect'].centerx, dm['rect'].centery - p2['rect'].centery)
                    if mesafe <= parry_radius:
                        p1['mermiler'].remove(dm)
                        dm['vx'] *= -2.2; dm['vy'] *= -2.2; dm['hasar'] = 15
                        p2['mermiler'].append(dm)
                        shake_amount = 15
                        parcacik_ekle(dm['rect'].centerx, dm['rect'].centery, SARI, 20)
                        if ses_atis: ses_atis.play()
                        break
            cd_kalan2 = max(0, silah_beklemeleri.get(p2['silah'], 3) - (su_an - p2['son_ates']))
            if gamepad_ates_basili(gp2) and p2['silah'] in ("tabanca", "pompali") and cd_kalan2 == 0:
                aci = nisan_acisi(p2['rect'].centerx, p2['rect'].centery, gp2, hedef=(p1['rect'].centerx, p1['rect'].centery))
                ates_et(p2, aci, "P1")
            if p2['silah'] == "taramali" and gamepad_ates_basili(gp2) and cd_kalan2 == 0:
                aci = nisan_acisi(p2['rect'].centerx, p2['rect'].centery, gp2, hedef=(p1['rect'].centerx, p1['rect'].centery))
                ates_et(p2, aci, "P1")
            gp_hareket2 = gamepad_hareket_x(gp2)
            if gp_hareket2 < 0: p2['rect'].x -= 5
            if gp_hareket2 > 0: p2['rect'].x += 5
            p2['rect'].x = max(0, min(EN - 30, p2['rect'].x))
            p2['y_hiz'] += 0.8; p2['rect'].y += p2['y_hiz']
            p2['yerde'] = False
            for p in platformlar:
                if p2['rect'].colliderect(p) and p2['y_hiz'] > 0:
                    p2['rect'].bottom, p2['y_hiz'], p2['yerde'] = p.top, 0, True
            if p2['rect'].y > BOY + 100: p2['rect'].x, p2['rect'].y, p2['y_hiz'] = EN - 130, 500, 0

            # --- mermi hareketleri + çarpışmalar ---
            for o, hedef in ((p1, p2), (p2, p1)):
                for m in o['mermiler'][:]:
                    m['rect'].x += m['vx']; m['rect'].y += m['vy']
                    if m['rect'].x < -20 or m['rect'].x > EN + 20 or m['rect'].y < -20 or m['rect'].y > BOY + 20:
                        o['mermiler'].remove(m); continue
                    if m['rect'].colliderect(hedef['rect']):
                        o['mermiler'].remove(m)
                        hedef['can'] -= m['hasar']
                        shake_amount = 10
                        parcacik_ekle(hedef['rect'].centerx, hedef['rect'].centery, PARLAK_KIRMIZI, 10)
                        if hedef['can'] <= 0:
                            kazandi = 1 if hedef is p2 else 2

        # --- ÇİZİM ---
        ekran.fill(SIYAH)
        for p in platformlar: pygame.draw.rect(ekran, (90, 20, 20), p)
        parcacik_guncelle()

        if resim_yuklendi: ekran.blit(V1_GORSEL, (p1['rect'].x, p1['rect'].y))
        else: pygame.draw.rect(ekran, YESIL, p1['rect'])
        if resim2_yuklendi: ekran.blit(V2_GORSEL, (p2['rect'].x, p2['rect'].y))
        else: pygame.draw.rect(ekran, TURKUAZ, p2['rect'])
        for m in p1['mermiler']: pygame.draw.rect(ekran, SARI, m['rect'])
        for m in p2['mermiler']: pygame.draw.rect(ekran, PARLAK_KIRMIZI, m['rect'])

        # Parry menzil halkası: sadece bir mermi menzile (parry mesafesinin biraz dışına) girince görünsün
        if kazandi is None:
            for m in p2['mermiler']:
                if math.hypot(m['rect'].centerx - p1['rect'].centerx, m['rect'].centery - p1['rect'].centery) <= parry_radius * 1.6:
                    pygame.draw.circle(ekran, SARI, p1['rect'].center, parry_radius, 1)
                    break
            for m in p1['mermiler']:
                if math.hypot(m['rect'].centerx - p2['rect'].centerx, m['rect'].centery - p2['rect'].centery) <= parry_radius * 1.6:
                    pygame.draw.circle(ekran, SARI, p2['rect'].center, parry_radius, 1)
                    break

        pygame.draw.rect(ekran, GRI, (20, 20, 200, 16))
        pygame.draw.rect(ekran, YESIL, (20, 20, 200 * max(0, p1['can']) / 100, 16))
        ekran.blit(font_ui.render("OYUNCU 1", True, YESIL), (20, 40))
        pygame.draw.rect(ekran, GRI, (EN - 220, 20, 200, 16))
        pygame.draw.rect(ekran, TURKUAZ, (EN - 220, 20, 200 * max(0, p2['can']) / 100, 16))
        ekran.blit(font_ui.render("OYUNCU 2", True, TURKUAZ), (EN - 220, 40))

        ipucu = font_ui.render(f"P1 SİLAH: {SILAH_ISIMLERI.get(p1['silah'])}   P2 SİLAH: {SILAH_ISIMLERI.get(p2['silah'])}", True, SARI)
        ekran.blit(ipucu, (EN//2 - ipucu.get_width()//2, BOY - 30))

        if kazandi is not None:
            t = font_baslik.render(f"OYUNCU {kazandi} KAZANDI!", True, (YESIL if kazandi == 1 else TURKUAZ))
            ekran.blit(t, (EN//2 - t.get_width()//2, 230))
            ip2 = font_ui.render("R: Tekrar Oyna   ESC: Ana Menü", True, GRI)
            ekran.blit(ip2, (EN//2 - ip2.get_width()//2, 300))

        scanline_ciz(); ekrani_guncelle(); saat.tick(60)


def yerel_futbol_dongusu(sema):
    """Aynı bilgisayarda, tek pencerede 2 kişilik futbol. Ağ bağlantısı YOK — top fiziği
    de dahil her şey tek süreçte, aynı ekranda simüle ediliyor.
    sema: 'kb_gp' -> P1 klavye+fare, P2 gamepad(0) | 'gp_gp' -> P1 gamepad(0), P2 gamepad(1)."""
    global shake_amount
    p1_gp_idx = None if sema == "kb_gp" else 0
    p2_gp_idx = 0 if sema == "kb_gp" else 1
    ...
    zemin = pygame.Rect(0, 550, EN, 50)
    ksy, kagy, kale_yukseklik, plat_verisi = _futbol_harita_uret()
    platformlar = [zemin] + [pygame.Rect(p[0], p[1], p[2], p[3]) for p in plat_verisi]
    kale_sol = pygame.Rect(0, ksy, 14, kale_yukseklik)
    kale_sag = pygame.Rect(EN - 14, kagy, 14, kale_yukseklik)
    ...
    def yeni_oyuncu_durumu(x):
        return {'rect': pygame.Rect(x, 500, 30, 30), 'y_hiz': 0, 'yerde': False,
                'son_ziplama': 0, 'son_ates': 0, 'mermiler': [], 'gp_zipla_onceki': False, 'gp_ates_onceki': False}

    p1 = yeni_oyuncu_durumu(100)
    p2 = yeni_oyuncu_durumu(EN - 130)
    top_x, top_y = EN / 2.0, 200.0
    top_vx, top_vy = 0.0, 0.0
    top_tasiyan = None  # None / 'p1' / 'p2'
    skor_p1, skor_p2 = 0, 0
    kazandi = None  # None: devam, 1: P1, 2: P2

    muzik_cal("oyun1.mp3")

    def vurus_isle(hedef_taraf_adi, hedef_rect):
        nonlocal top_tasiyan, top_x, top_y, top_vx, top_vy
        parcacik_ekle(hedef_rect.centerx, hedef_rect.centery, PARLAK_KIRMIZI, 25)
        shake_amount = 12
        if top_tasiyan == hedef_taraf_adi:
            top_tasiyan = None
            top_x, top_y = hedef_rect.centerx, hedef_rect.top - 22
            top_vx, top_vy = random.uniform(-4, 4), -6
        if hedef_taraf_adi == 'p1':
            p1['rect'].x, p1['rect'].y, p1['y_hiz'] = 100, 500, 0
        else:
            p2['rect'].x, p2['rect'].y, p2['y_hiz'] = EN - 130, 500, 0
    ...
    while True:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN and event.key == pygame.K_ESCAPE: return
            if kazandi is not None and event.type == pygame.KEYDOWN and event.key == pygame.K_r:
                p1 = yeni_oyuncu_durumu(100); p2 = yeni_oyuncu_durumu(EN - 130)
                top_x, top_y, top_vx, top_vy, top_tasiyan = EN/2.0, 200.0, 0.0, 0.0, None
                skor_p1, skor_p2, kazandi = 0, 0, None
            if kazandi is None and sema == "kb_gp" and event.type == pygame.KEYDOWN and event.key == pygame.K_w:
                if su_an - p1['son_ziplama'] > 0.1:
                    if p1['yerde']: p1['y_hiz'], p1['yerde'] = -15, False
                    p1['son_ziplama'] = su_an
            if kazandi is None and sema == "kb_gp" and event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                aci = nisan_acisi(p1['rect'].centerx, p1['rect'].centery, None, 0, ham_fare=True)
                if top_tasiyan == 'p1':
                    top_tasiyan = None
                    top_vx, top_vy = math.cos(aci) * 22, math.sin(aci) * 22
                    parcacik_ekle(p1['rect'].centerx, p1['rect'].centery, SARI, 6); shake_amount = 4
                elif su_an - p1['son_ates'] > silah_beklemesi:
                    p1['mermiler'].append({'rect': pygame.Rect(p1['rect'].centerx, p1['rect'].centery, 10, 10),
                                            'vx': math.cos(aci)*14, 'vy': math.sin(aci)*14})
                    p1['son_ates'] = su_an
                    if ses_atis: ses_atis.play()
                    shake_amount = 5
        ...
        if kazandi is None:
            ...
            if sema == "kb_gp":
                ...
                if zip_tetik and su_an - p1['son_ziplama'] > 0.1:
                    if p1['yerde']: p1['y_hiz'], p1['yerde'] = -15, False
                    p1['son_ziplama'] = su_an
                gp1_ates_simdi = gamepad_ates_basili(gp1)
                gp1_ates_tetik = gp1_ates_simdi and not p1['gp_ates_onceki']
                p1['gp_ates_onceki'] = gp1_ates_simdi
                if gp1_ates_tetik:
                    hedef1 = (kale_sag.centerx, kale_sag.centery) if top_tasiyan == 'p1' else (p2['rect'].centerx, p2['rect'].centery)
                    aci = nisan_acisi(p1['rect'].centerx, p1['rect'].centery, gp1, hedef=hedef1)
                    if top_tasiyan == 'p1':
                        top_tasiyan = None
                        top_vx, top_vy = math.cos(aci) * 22, math.sin(aci) * 22
                        parcacik_ekle(p1['rect'].centerx, p1['rect'].centery, SARI, 6); shake_amount = 4
            ...
            if zip_tetik2 and su_an - p2['son_ziplama'] > 0.1:
                if p2['yerde']: p2['y_hiz'], p2['yerde'] = -15, False
                p2['son_ziplama'] = su_an
            gp2_ates_simdi = gamepad_ates_basili(gp2)
            gp2_ates_tetik = gp2_ates_simdi and not p2['gp_ates_onceki']
            p2['gp_ates_onceki'] = gp2_ates_simdi
            if gp2_ates_tetik:
                hedef2 = (kale_sol.centerx, kale_sol.centery) if top_tasiyan == 'p2' else (p1['rect'].centerx, p1['rect'].centery)
                aci = nisan_acisi(p2['rect'].centerx, p2['rect'].centery, gp2, hedef=hedef2)
                if top_tasiyan == 'p2':
                    top_tasiyan = None
                    top_vx, top_vy = math.cos(aci) * 22, math.sin(aci) * 22
                    parcacik_ekle(p2['rect'].centerx, p2['rect'].centery, SARI, 6); shake_amount = 4
                elif su_an - p2['son_ates'] > silah_beklemesi:
                    p2['mermiler'].append({'rect': pygame.Rect(p2['rect'].centerx, p2['rect'].centery, 10, 10),
                                            'vx': math.cos(aci)*14, 'vy': math.sin(aci)*14})
                    p2['son_ates'] = su_an
                    if ses_atis: ses_atis.play()
                    shake_amount = 5
            ...
            # --- mermiler: hareket + çarpışma (tek can, isabet = anında yeniden doğma) ---
            for o, hedef_adi, hedef in ((p1, 'p2', p2), (p2, 'p1', p1)):
                for m in o['mermiler'][:]:
                    m['rect'].x += m['vx']; m['rect'].y += m['vy']
                    if m['rect'].x < -20 or m['rect'].x > EN + 20 or m['rect'].y < -20 or m['rect'].y > BOY + 20:
                        o['mermiler'].remove(m); continue
                    if m['rect'].colliderect(hedef['rect']):
                        o['mermiler'].remove(m)
                        vurus_isle(hedef_adi, hedef['rect'])

            # --- top fiziği ---
            gol_oldu = None
            if top_tasiyan == 'p1':
                top_x, top_y = p1['rect'].centerx, p1['rect'].top - 22
            elif top_tasiyan == 'p2':
                top_x, top_y = p2['rect'].centerx, p2['rect'].top - 22
            else:
                top_vy += 0.7
                top_x += top_vx; top_y += top_vy
                top_rect = pygame.Rect(int(top_x - top_yaricap), int(top_y - top_yaricap), top_yaricap*2, top_yaricap*2)
                if top_rect.colliderect(kale_sol):
                    gol_oldu = 'p2'
                elif top_rect.colliderect(kale_sag):
                    gol_oldu = 'p1'
                else:
                    for p in platformlar:
                        if top_rect.colliderect(p) and top_vy >= 0 and (top_y - top_yaricap) < p.top:
                            top_y = p.top - top_yaricap
                            if abs(top_vy) > 1: top_vy = -top_vy * 0.55
                            else: top_vy = 0
                            top_vx *= 0.85
                            break
                    if top_y - top_yaricap <= 0: top_y = top_yaricap; top_vy = -top_vy * 0.5
                    if top_x - top_yaricap <= 0: top_x = top_yaricap; top_vx = -top_vx * 0.6
                    elif top_x + top_yaricap >= EN: top_x = EN - top_yaricap; top_vx = -top_vx * 0.6

                    if math.hypot(top_x - p1['rect'].centerx, top_y - p1['rect'].centery) <= 34:
                        top_tasiyan = 'p1'; top_vx = top_vy = 0
                    elif math.hypot(top_x - p2['rect'].centerx, top_y - p2['rect'].centery) <= 34:
                        top_tasiyan = 'p2'; top_vx = top_vy = 0

            if gol_oldu == 'p1': skor_p1 += 1
            elif gol_oldu == 'p2': skor_p2 += 1
            if gol_oldu:
                top_x, top_y, top_vx, top_vy, top_tasiyan = EN/2.0, 200.0, 0.0, 0.0, None
                shake_amount = 18
                parcacik_ekle(EN/2, 200, SARI, 30)

            if skor_p1 >= HEDEF_SKOR: kazandi = 1
            elif skor_p2 >= HEDEF_SKOR: kazandi = 2

        # --- ÇİZİM ---
        ekran.fill(SIYAH)
        for p in platformlar: pygame.draw.rect(ekran, (90, 20, 20), p)
        pygame.draw.rect(ekran, TURKUAZ, kale_sol, 3)
        pygame.draw.rect(ekran, MOR, kale_sag, 3)
        parcacik_guncelle()
        ...
        for m in p1['mermiler']: pygame.draw.rect(ekran, SARI, m['rect'])
        for m in p2['mermiler']: pygame.draw.rect(ekran, PARLAK_KIRMIZI, m['rect'])
        pygame.draw.circle(ekran, SARI, (int(top_x), int(top_y)), top_yaricap)
        pygame.draw.circle(ekran, SIYAH, (int(top_x), int(top_y)), top_yaricap, 2)

        skor_yazi = font_baslik.render(f"{skor_p1} - {skor_p2}", True, BEYAZ)
        ekran.blit(skor_yazi, (EN//2 - skor_yazi.get_width()//2, 20))
        ipucu = font_ui.render("SOL TIK/RT: tasirken firlat, tasimazken ates et  |  HEDEF: " + str(HEDEF_SKOR) + " GOL", True, GRI)
        ekran.blit(ipucu, (EN//2 - ipucu.get_width()//2, 70))

        if kazandi is not None:
            t = font_baslik.render(f"OYUNCU {kazandi} KAZANDI!", True, (YESIL if kazandi == 1 else TURKUAZ))
            ekran.blit(t, (EN//2 - t.get_width()//2, 230))
            ip2 = font_ui.render("R: Tekrar Oyna   ESC: Ana Menu", True, GRI)
            ekran.blit(ip2, (EN//2 - ip2.get_width()//2, 300))

        scanline_ciz(); ekrani_guncelle(); saat.tick(60)


def mp_dongusu_baslat(conn, is_host, rakip_adi="RAKİP"):
    """Seçili moda (MP_MOD) göre doğru oyun döngüsünü başlatır."""
    if MP_MOD == "futbol":
        mp_futbol_dongusu(conn, is_host, rakip_adi)
    else:
        mp_oyun_dongusu(conn, is_host, rakip_adi)
...
def mp_sunucu_kur():
    ...
    while True:
        ...
        if kutu["conn"]:
            mp_dongusu_baslat(kutu["conn"], True)
            try: sunucu_soket.close()
            except Exception: pass
            return
        if kutu["hata"]:
...
def mp_alici_thread(conn, kutu):
    ...
    try:
        ...
        for satir in dosya:
            ...
            # Sunucu, rakip ayrılınca bağlantıyı KAPATMADAN sadece bu bilgi mesajını gönderiyor
            # (relay üzerinden internet bağlantısında). Bunu görünce biz kendimiz bağlantıyı
            # koptu say ıyoruz — yoksa TCP açık kaldığı için hiç fark edilmiyordu.
            if veri.get('sys') == 'rakip_ayrildi':
            ...
            elif tip == 'top':  # sadece host gönderir, top'un yetkili (authoritative) durumu
                kutu['top'] = veri
            elif tip == 'topat':  # sadece client gönderir, "topu fırlattım" bildirimi (host uygular)
                kutu.setdefault('gelen_top_atis', []).append(veri)
            elif tip == 'harita':  # sadece host gönderir, rastgele saha düzeni (platform+kale konumları)
                kutu['harita'] = veri
            elif tip == 'topvurus':  # bir mermi karşı tarafa isabet etti (topu düşürsün diye)
                kutu.setdefault('gelen_top_vurus', 0)
                kutu['gelen_top_vurus'] += 1
            elif tip == 'rematch':  # karşı taraf "tekrar oyna" dedi
                kutu['rakip_rematch_hazir'] = True
            elif tip == 'profil':  # karşı tarafın kullanıcı adı (LAN modunda kullanılır)
                kutu['rakip_adi'] = veri.get('ad', 'RAKİP')
    except Exception:
        pass
    kutu['baglanti_koptu'] = True

def mp_oyun_dongusu(conn, is_host, rakip_adi="RAKİP"):
    global shake_amount
    conn.settimeout(None)
    kutu = {'x': (EN - 100 if is_host else 100), 'y': 500, 'can': 100,
            'gelen_hasar': 0, 'rakip_oldu': False, 'baglanti_koptu': False, 'rakip_adi': rakip_adi}
    threading.Thread(target=mp_alici_thread, args=(conn, kutu), daemon=True).start()
    try: conn.sendall((json.dumps({'t': 'profil', 'ad': KULLANICI_ADI}) + "\n").encode())
    except Exception: pass
    ...
    # --- GAMEPAD DURUMU (multiplayer'da tüm silahlar açık olduğu için kilit kontrolü yok) ---
    gp_ziplama_basili_onceki = False
    gp_ates_basili_onceki = False
    gp_parry_basili_onceki = False
    gp_silah_lb_onceki, gp_silah_rb_onceki = False, False
    ...
    while True:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN and event.key == pygame.K_ESCAPE:
                ...
                return
            if kazandi is not None and not kendi_rematch_hazir and event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                btn_rematch = pygame.Rect(EN//2 - 110, 280, 220, 44)
                if btn_rematch.collidepoint(fare_konumu()):
                    kendi_rematch_hazir = True
                    try: conn.sendall((json.dumps({'t': 'rematch'}) + "\n").encode())
        ...
        if kazandi is None:
            ...
            gp_silah_yon, gp_silah_lb_onceki, gp_silah_rb_onceki = gamepad_silah_degistir_tetiklendi(gp, gp_silah_lb_onceki, gp_silah_rb_onceki)
            if gp_silah_yon != 0:
                idx = (SILAH_SIRASI.index(secili_silah) + gp_silah_yon) % len(SILAH_SIRASI)
                secili_silah = SILAH_SIRASI[idx]
            ...
            ates_cd_kalan_gp = max(0, silah_beklemeleri.get(secili_silah, 3) - (su_an - son_ates))
            if gamepad_ates_basili(gp) and secili_silah in ("tabanca", "pompali") and ates_cd_kalan_gp == 0:
                aci = nisan_acisi(oyuncu.centerx, oyuncu.centery, gp, hedef=(rakip_rect.centerx, rakip_rect.centery))
                if secili_silah == "tabanca":
                    vx, vy = math.cos(aci) * 12, math.sin(aci) * 12
                    mermiler.append({'rect': pygame.Rect(oyuncu.centerx, oyuncu.centery, 10, 10), 'vx': vx, 'vy': vy, 'hasar': 10})
                    try:
        ...
        # --- TARAMALI SİLAH: basılı tutarak sürekli ateş ---
        ates_cd_kalan = max(0, silah_beklemeleri.get(secili_silah, 3) - (su_an - son_ates))
        if kazandi is None and secili_silah == "taramali" and (pygame.mouse.get_pressed()[0] or gamepad_ates_basili(gp)) and ates_cd_kalan == 0:
            aci = nisan_acisi(oyuncu.centerx, oyuncu.centery, gp, hedef=(rakip_rect.centerx, rakip_rect.centery))
        ...
        if kazandi is None:
            tus = pygame.key.get_pressed()
            gp_hareket = gamepad_hareket_x(gp)
            if tus[pygame.K_a] or gp_hareket < 0: oyuncu.x -= 5
            if tus[pygame.K_d] or gp_hareket > 0: oyuncu.x += 5
        ...
        # İkisi de "TEKRAR OYNA"ya bastıysa (ve bağlantı hâlâ sağlamsa) şimdi sıfırla ve yeni maça başla
        if kendi_rematch_hazir and kutu.get('rakip_rematch_hazir') and not kutu.get('baglanti_koptu'):
            oyuncu = pygame.Rect(100 if is_host else EN - 130, 500, 30, 30)
            oyuncu_y_hiz = 0; oyuncu_yerde = False
            son_ziplama_girdisi = 0
            can = 100
            ...
            son_ates = 0
            kendi_rematch_hazir = False
            kutu['rakip_oldu'] = False
            kutu['gelen_hasar'] = 0
            kutu['rakip_rematch_hazir'] = False
            # Rakibin ekrandaki (yumuşatılmış) konumunu da anında doğru spawn noktasına sabitle —
            # yoksa eski konumda takılı kalıp "aynı yerden başlamış" gibi görünüyordu.
            rakip_gorsel_x = float(EN - 130 if is_host else 100)
            rakip_gorsel_y = 500.0
            kutu['x'] = rakip_gorsel_x; kutu['y'] = rakip_gorsel_y
        ...
        ekran.blit(font_ui.render(KULLANICI_ADI, True, YESIL), (20, 40))

        rakip_can = kutu.get('can', 100)
        rakip_adi_goster = kutu.get('rakip_adi', rakip_adi)
        pygame.draw.rect(ekran, GRI, (EN - 220, 20, 200, 16))
        pygame.draw.rect(ekran, TURKUAZ, (EN - 220, 20, 200 * max(0, rakip_can) / 100, 16))
        ekran.blit(font_ui.render(rakip_adi_goster, True, TURKUAZ), (EN - 220, 40))
        ...
        if kazandi is True:
            ...
            ekran.blit(t, (EN // 2 - t.get_width() // 2, 200))
        if kazandi is not None:
            if kendi_rematch_hazir:
                bekleme = font_buton.render("Rakip bekleniyor...", True, SARI)
                ekran.blit(bekleme, (EN//2 - bekleme.get_width()//2, 280))
            else:
                btn_rematch = pygame.Rect(EN//2 - 110, 280, 220, 44)
                hov = btn_rematch.collidepoint(fare_konumu())
                rc = SARI if hov else BEYAZ
                pygame.draw.rect(ekran, rc, btn_rematch, 2)
                rt = font_buton.render("TEKRAR OYNA", True, rc)
                ekran.blit(rt, (btn_rematch.centerx - rt.get_width()//2, btn_rematch.centery - 11))
                if kutu.get('rakip_rematch_hazir'):
                    rh = font_ui.render("(Rakip zaten hazır!)", True, YESIL)
                    ekran.blit(rh, (EN//2 - rh.get_width()//2, 330))
            ipucu = font_ui.render("ESC: Ana Menü", True, GRI)
            ekran.blit(ipucu, (EN // 2 - ipucu.get_width() // 2, 360))

        scanline_ciz(); ekrani_guncelle(); saat.tick(60)

def _futbol_harita_uret():
    """Rastgele ama oynanabilir bir saha düzeni üretir: platform konumları + kale yükseklikleri.
    Host bunu bir kere üretir, client'a gönderir; ikisi de AYNI düzeni kullanır (senkron kalsın diye)."""
    kale_yukseklik = 100
    ksy = random.randint(160, 550 - kale_yukseklik - 20)   # sol kale (host'un kalesi) y konumu
    kagy = random.randint(160, 550 - kale_yukseklik - 20)  # sağ kale (client'ın kalesi) y konumu
    platlar = []
    for _ in range(3):
        w = random.randint(80, 150)
        x = random.randint(130, EN - 130 - w)
        y = random.randint(280, 480)
        platlar.append([x, y, w, 16])
    return ksy, kagy, kale_yukseklik, platlar


def mp_futbol_dongusu(conn, is_host, rakip_adi="RAKİP"):
    """2 kişilik futbol modu: ortadan bir top düşer, yakalayıp rakibin kalesine atmak amaç.
    Top fiziği HOST tarafında yetkilidir (authoritative) — client sadece host'un yayınladığı
    top durumunu gösterir ve kendi atışını 'topat' mesajıyla host'a bildirir. Saha düzeni
    (platformlar + kale yükseklikleri) her maçta host tarafından rastgele üretilip client'a
    'harita' mesajıyla gönderilir, ikisi de aynı düzeni kullanır."""
    global shake_amount
    conn.settimeout(None)
    kutu = {'x': (EN - 100 if is_host else 100), 'y': 500, 'can': 100,
            'gelen_hasar': 0, 'rakip_oldu': False, 'baglanti_koptu': False,
            'top': None, 'gelen_top_atis': [], 'harita': None, 'gelen_top_vurus': 0, 'rakip_adi': rakip_adi}
    threading.Thread(target=mp_alici_thread, args=(conn, kutu), daemon=True).start()
    try: conn.sendall((json.dumps({'t': 'profil', 'ad': KULLANICI_ADI}) + "\n").encode())
    except Exception: pass
    ...
    if is_host:
        ...
        # Client, host'tan gelene kadar varsayılan (geçici) bir düzenle başlar
        ksy, kagy, kale_yukseklik, plat_verisi = 260, 260, 100, []

    def haritayi_kur(ksy, kagy, kale_yukseklik, plat_verisi):
        platformlar = [zemin] + [pygame.Rect(p[0], p[1], p[2], p[3]) for p in plat_verisi]
        kale_sol = pygame.Rect(0, ksy, 14, kale_yukseklik)
        kale_sag = pygame.Rect(EN - 14, kagy, 14, kale_yukseklik)
        return platformlar, kale_sol, kale_sag

    platformlar, kale_sol, kale_sag = haritayi_kur(ksy, kagy, kale_yukseklik, plat_verisi)
    ...
    son_gonderim = 0
    top_yaricap = 12
    harita_gonderim_sayaci = 0  # host, ilk saniyede haritayı birkaç kez tekrar gönderir (garantiye almak için)

    rakip_gorsel_x = float(kutu.get('x', EN - 100))
    rakip_gorsel_y = float(kutu.get('y', 500))

    top_x, top_y = EN / 2, 200.0
    top_vx, top_vy = 0.0, 0.0
    top_tasiyan = None
    skor_host, skor_client = 0, 0
    top_gorsel_x, top_gorsel_y = top_x, top_y
    ...
    while True:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN and event.key == pygame.K_ESCAPE:
                ...
                return
            if kazandi is not None and not kendi_rematch_hazir and event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                btn_rematch = pygame.Rect(EN//2 - 110, 300, 220, 44)
                if btn_rematch.collidepoint(fare_konumu()):
                    kendi_rematch_hazir = True
                    try: conn.sendall((json.dumps({'t': 'rematch'}) + "\n").encode())
                    except Exception: kutu['baglanti_koptu'] = True
            ...
            if kazandi is None and event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                benim_taraf = 'host' if is_host else 'client'
                if top_tasiyan == benim_taraf:
                    # --- FIRLATMA: topu taşıyorsan düz/hızlı fırlat ---
                    fx, fy = fare_konumu()
                    aci = math.atan2(fy - oyuncu.centery, fx - oyuncu.centerx)
                    firlatma_hizi = 22
                    vx, vy = math.cos(aci) * firlatma_hizi, math.sin(aci) * firlatma_hizi
                    if is_host:
                        top_tasiyan = None
                        top_vx, top_vy = vx, vy
                    else:
                        try:
                            conn.sendall((json.dumps({'t': 'topat', 'vx': vx, 'vy': vy}) + "\n").encode())
                        except Exception:
                            kutu['baglanti_koptu'] = True
                    parcacik_ekle(oyuncu.centerx, oyuncu.centery, SARI, 6)
                    shake_amount = 4
                elif su_an - son_ates > silah_beklemesi:
                    # --- ATEŞ: topu taşımıyorsan silahınla vurup rakibin topu düşürmesini sağla ---
                    fx, fy = fare_konumu()
                    aci = math.atan2(fy - oyuncu.centery, fx - oyuncu.centerx)
                    vx, vy = math.cos(aci) * 14, math.sin(aci) * 14
                    mermiler.append({'rect': pygame.Rect(oyuncu.centerx, oyuncu.centery, 10, 10), 'vx': vx, 'vy': vy})
                    try:
                        conn.sendall((json.dumps({'t': 'atis', 'x': oyuncu.centerx, 'y': oyuncu.centery, 'vx': vx, 'vy': vy}) + "\n").encode())
                    except Exception:
                        kutu['baglanti_koptu'] = True
        ...
        if kazandi is None:
            ...
            gp_ates_simdi = gamepad_ates_basili(gp)
            gp_ates_tetik = gp_ates_simdi and not gp_ates_basili_onceki
            gp_ates_basili_onceki = gp_ates_simdi
            if gp_ates_tetik:
                benim_taraf = 'host' if is_host else 'client'
                if top_tasiyan == benim_taraf:
                    hedef_kale = kale_sag if is_host else kale_sol
                    hedef_konum = (hedef_kale.centerx, hedef_kale.centery)
                else:
                    hedef_konum = (rakip_rect.centerx, rakip_rect.centery)
                aci = nisan_acisi(oyuncu.centerx, oyuncu.centery, gp, hedef=hedef_konum)
                if top_tasiyan == benim_taraf:
                    firlatma_hizi = 22
                    vx, vy = math.cos(aci) * firlatma_hizi, math.sin(aci) * firlatma_hizi
                    if is_host:
        ...
        if kazandi is None:
            ...
            if oyuncu.y > BOY + 100:
                oyuncu.x, oyuncu.y, oyuncu_y_hiz = EN // 2, 400, 0

        rakip_rect = pygame.Rect(int(rakip_gorsel_x), int(rakip_gorsel_y), 30, 30)
        hedef_x = kutu.get('x', rakip_gorsel_x); hedef_y = kutu.get('y', rakip_gorsel_y)
        rakip_gorsel_x += (hedef_x - rakip_gorsel_x) * 0.25
        rakip_gorsel_y += (hedef_y - rakip_gorsel_y) * 0.25
        rakip_rect = pygame.Rect(int(rakip_gorsel_x), int(rakip_gorsel_y), 30, 30)

        # --- CLIENT: host'tan harita geldiyse uygula ---
        if not is_host:
            gelen_harita = kutu.get('harita')
            if gelen_harita:
                ksy = gelen_harita.get('ksy', ksy); kagy = gelen_harita.get('kagy', kagy)
                kale_yukseklik = gelen_harita.get('ky', kale_yukseklik)
                plat_verisi = gelen_harita.get('plat', plat_verisi)
                platformlar, kale_sol, kale_sag = haritayi_kur(ksy, kagy, kale_yukseklik, plat_verisi)
                kutu['harita'] = None  # tekrar tekrar yeniden kurmayı önle

        # --- HOST: ilk saniye haritayı birkaç kez gönder (client'ın kaçırma ihtimaline karşı) ---
        if is_host and harita_gonderim_sayaci < 45:
            try:
                conn.sendall((json.dumps({'t': 'harita', 'ksy': ksy, 'kagy': kagy, 'ky': kale_yukseklik, 'plat': plat_verisi}) + "\n").encode())
            except Exception:
                kutu['baglanti_koptu'] = True
            harita_gonderim_sayaci += 1

        # --- MERMİLER: kendi mermilerim ---
        for m in mermiler[:]:
            m['rect'].x += m['vx']; m['rect'].y += m['vy']
            if m['rect'].x < -20 or m['rect'].x > EN + 20 or m['rect'].y < -20 or m['rect'].y > BOY + 20:
                mermiler.remove(m); continue
            if kazandi is None and m['rect'].colliderect(rakip_rect):
                mermiler.remove(m)
                try:
                    conn.sendall((json.dumps({'t': 'topvurus'}) + "\n").encode())
                    conn.sendall((json.dumps({'t': 'hit', 'dmg': 1}) + "\n").encode())
                except Exception: kutu['baglanti_koptu'] = True
                if is_host and top_tasiyan == 'client':
                    top_tasiyan = None
                    top_x, top_y = rakip_rect.centerx, rakip_rect.top - 22
                    top_vx, top_vy = random.uniform(-4, 4), -6
                parcacik_ekle(rakip_rect.centerx, rakip_rect.centery, SARI, 10)
                shake_amount = 6

        # --- rakibin mermileri (görsel) ---
        yeni_mermiler = kutu.get('gelen_mermiler')
        if yeni_mermiler:
            for vm in yeni_mermiler:
                rakip_mermiler.append({'rect': pygame.Rect(int(vm.get('x', 0)), int(vm.get('y', 0)), 10, 10),
        ...
        for m in rakip_mermiler[:]:
            ...
            if kazandi is None and m['rect'].colliderect(oyuncu):
                rakip_mermiler.remove(m)
                parcacik_ekle(oyuncu.centerx, oyuncu.centery, PARLAK_KIRMIZI, 10)

        # --- CAN: vurulunca azal, biterse yeniden doğ (kendi tarafına, topu varsa düşer) ---
        if kutu.get('gelen_hasar', 0) > 0 and kazandi is None:
            can -= kutu['gelen_hasar']
            kutu['gelen_hasar'] = 0
            shake_amount = 10
            if can <= 0:
                can = can_maks
                oyuncu.x, oyuncu.y = (100 if is_host else EN - 130), 500
                oyuncu_y_hiz = 0
                if is_host and top_tasiyan == 'host':
                    top_tasiyan = None
                    top_x, top_y = oyuncu.centerx, oyuncu.top - 22
                    top_vx, top_vy = random.uniform(-4, 4), -6
                parcacik_ekle(oyuncu.centerx, oyuncu.centery, PARLAK_KIRMIZI, 25)

        gol_oldu_bu_kare = None

        if is_host:
            client_gelen_atis = kutu.get('gelen_top_atis')
            if client_gelen_atis and top_tasiyan == 'client':
                son = client_gelen_atis[-1]
                top_tasiyan = None
                top_vx, top_vy = son.get('vx', 0), son.get('vy', 0)
                kutu['gelen_top_atis'] = []
            elif client_gelen_atis:
                kutu['gelen_top_atis'] = []

            # client'tan "vurdum" bildirimi geldiyse ve host topu taşıyorsa düşür
            if kutu.get('gelen_top_vurus', 0) > 0 and top_tasiyan == 'host':
                top_tasiyan = None
                top_x, top_y = oyuncu.centerx, oyuncu.top - 22
                top_vx, top_vy = random.uniform(-4, 4), -6
            kutu['gelen_top_vurus'] = 0

            if top_tasiyan == 'host':
                top_x, top_y = oyuncu.centerx, oyuncu.top - 22
            elif top_tasiyan == 'client':
                top_x, top_y = rakip_rect.centerx, rakip_rect.top - 22
            else:
                top_vy += 0.7
                top_x += top_vx; top_y += top_vy
                top_rect_gecici = pygame.Rect(int(top_x - top_yaricap), int(top_y - top_yaricap), top_yaricap*2, top_yaricap*2)

                if top_rect_gecici.colliderect(kale_sol):
                    gol_oldu_bu_kare = 'client'
                elif top_rect_gecici.colliderect(kale_sag):
                    gol_oldu_bu_kare = 'host'
                else:
                    # tüm platformlara (zemin dahil) sekme
                    for p in platformlar:
                        if top_rect_gecici.colliderect(p) and top_vy >= 0 and (top_y - top_yaricap) < p.top:
                            top_y = p.top - top_yaricap
                            if abs(top_vy) > 1: top_vy = -top_vy * 0.55
                            else: top_vy = 0
                            top_vx *= 0.85
                            break
                    if top_y - top_yaricap <= 0:
                        top_y = top_yaricap; top_vy = -top_vy * 0.5
                    if top_x - top_yaricap <= 0:
                        top_x = top_yaricap; top_vx = -top_vx * 0.6
                    elif top_x + top_yaricap >= EN:
                        top_x = EN - top_yaricap; top_vx = -top_vx * 0.6

                    if math.hypot(top_x - oyuncu.centerx, top_y - oyuncu.centery) <= 34:
                        top_tasiyan = 'host'; top_vx = top_vy = 0
                    elif math.hypot(top_x - rakip_rect.centerx, top_y - rakip_rect.centery) <= 34:
                        top_tasiyan = 'client'; top_vx = top_vy = 0

            if gol_oldu_bu_kare == 'host': skor_host += 1
            elif gol_oldu_bu_kare == 'client': skor_client += 1
            if gol_oldu_bu_kare:
                top_x, top_y, top_vx, top_vy, top_tasiyan = EN / 2, 200.0, 0.0, 0.0, None
                shake_amount = 18
                parcacik_ekle(EN/2, 200, SARI, 30)

            if skor_host >= HEDEF_SKOR and kazandi is None: kazandi = True
            elif skor_client >= HEDEF_SKOR and kazandi is None: kazandi = False

            try:
                conn.sendall((json.dumps({'t': 'top', 'x': top_x, 'y': top_y, 'vx': top_vx, 'vy': top_vy,
                                           'tasiyan': top_tasiyan, 'sh': skor_host, 'sc': skor_client}) + "\n").encode())
            except Exception:
                kutu['baglanti_koptu'] = True

            top_gorsel_x, top_gorsel_y = top_x, top_y
            benim_skor, rakip_skor = skor_host, skor_client
        else:
            gelen_top = kutu.get('top')
            if gelen_top:
                top_x, top_y = gelen_top.get('x', top_x), gelen_top.get('y', top_y)
                top_tasiyan = gelen_top.get('tasiyan')
                skor_host = gelen_top.get('sh', skor_host)
                skor_client = gelen_top.get('sc', skor_client)
                if skor_host >= HEDEF_SKOR and kazandi is None: kazandi = False
                elif skor_client >= HEDEF_SKOR and kazandi is None: kazandi = True

            if top_tasiyan == 'client':
                top_gorsel_x, top_gorsel_y = oyuncu.centerx, oyuncu.top - 22
            elif top_tasiyan == 'host':
                top_gorsel_x, top_gorsel_y = rakip_rect.centerx, rakip_rect.top - 22
            else:
                top_gorsel_x += (top_x - top_gorsel_x) * 0.35
                top_gorsel_y += (top_y - top_gorsel_y) * 0.35
            benim_skor, rakip_skor = skor_client, skor_host
        ...
        if kendi_rematch_hazir and kutu.get('rakip_rematch_hazir') and not kutu.get('baglanti_koptu'):
            ...
            if is_host:
                ksy, kagy, kale_yukseklik, plat_verisi = _futbol_harita_uret()
                platformlar, kale_sol, kale_sag = haritayi_kur(ksy, kagy, kale_yukseklik, plat_verisi)
                harita_gonderim_sayaci = 0

        if su_an - son_gonderim > 0.033:
            try:
                conn.sendall((json.dumps({'t': 'pos', 'x': oyuncu.x, 'y': oyuncu.y, 'can': can}) + "\n").encode())
            except Exception:
                kutu['baglanti_koptu'] = True
        ...
        if benim_var: ekran.blit(benim_gorsel, (oyuncu.x, oyuncu.y))
        else: pygame.draw.rect(ekran, YESIL, oyuncu)
        if rakip_var: ekran.blit(rakip_gorsel, (rakip_rect.x, rakip_rect.y))
        else: pygame.draw.rect(ekran, TURKUAZ, rakip_rect)

        benim_isim_yazi = font_ui.render(KULLANICI_ADI, True, YESIL)
        ekran.blit(benim_isim_yazi, (oyuncu.centerx - benim_isim_yazi.get_width()//2, oyuncu.top - 18))
        rakip_isim_yazi = font_ui.render(kutu.get('rakip_adi', rakip_adi), True, TURKUAZ)
        ekran.blit(rakip_isim_yazi, (rakip_rect.centerx - rakip_isim_yazi.get_width()//2, rakip_rect.top - 18))

        for m in mermiler: pygame.draw.rect(ekran, SARI, m['rect'])
        for m in rakip_mermiler: pygame.draw.rect(ekran, PARLAK_KIRMIZI, m['rect'])

        pygame.draw.circle(ekran, SARI, (int(top_gorsel_x), int(top_gorsel_y)), top_yaricap)
        pygame.draw.circle(ekran, SIYAH, (int(top_gorsel_x), int(top_gorsel_y)), top_yaricap, 2)

        # (tek can + anında yeniden doğma olduğu için can barı gösterilmiyor)

        skor_yazi = font_baslik.render(f"{benim_skor} - {rakip_skor}", True, BEYAZ)
        ekran.blit(skor_yazi, (EN//2 - skor_yazi.get_width()//2, 20))
        ipucu = font_ui.render("SOL TIK: taşırken fırlat, taşımazken ateş et  |  HEDEF: " + str(HEDEF_SKOR) + " GOL", True, GRI)
        ekran.blit(ipucu, (EN//2 - ipucu.get_width()//2, 70))

        if kazandi is True:
            t = font_baslik.render("KAZANDIN!", True, YESIL)
            ekran.blit(t, (EN // 2 - t.get_width() // 2, 220))
        elif kazandi is False:
            t = font_baslik.render("KAYBETTİN!", True, PARLAK_KIRMIZI)
            ekran.blit(t, (EN // 2 - t.get_width() // 2, 220))
        if kazandi is not None:
            if kendi_rematch_hazir:
                bekleme = font_buton.render("Rakip bekleniyor...", True, SARI)
                ekran.blit(bekleme, (EN//2 - bekleme.get_width()//2, 300))
            else:
                btn_rematch = pygame.Rect(EN//2 - 110, 300, 220, 44)
                hov = btn_rematch.collidepoint(fare_konumu())
                rc = SARI if hov else BEYAZ
                ...
                if kutu.get('rakip_rematch_hazir'):
                    rh = font_ui.render("(Rakip zaten hazır!)", True, YESIL)
                    ekran.blit(rh, (EN//2 - rh.get_width()//2, 350))
            ipucu2 = font_ui.render("ESC: Ana Menü", True, GRI)
            ekran.blit(ipucu2, (EN // 2 - ipucu2.get_width() // 2, 380))
