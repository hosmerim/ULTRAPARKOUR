...
MP_PORT = 5555
MP_MOD = "duello"  # "duello" veya "futbol" — mp_menu'deki mod düğmesiyle değişir
...
def mp_menu():
    ...
    while True:
        ...
        btn_mod = pygame.Rect(300, 118, 200, 34)
        ...
        btn_host = pygame.Rect(250, 225, 300, 44)
        btn_join = pygame.Rect(250, 278, 300, 44)
        btn_net  = pygame.Rect(250, 345, 300, 50)
        btn_geri = pygame.Rect(300, 430, 200, 44)

        lan_lbl = font_ui.render("— AYNI AĞ (LAN) —", True, GRI)
        ekran.blit(lan_lbl, (EN//2 - lan_lbl.get_width()//2, 208))
        net_lbl = font_ui.render("— İNTERNET (ODA KODU) —", True, MOR)
        ekran.blit(net_lbl, (EN//2 - net_lbl.get_width()//2, 328))
...
def mp_internet_baglan():
    ...
    while True:
        ...
        if kutu_baglanti["durum"] == "hazir":
            ...
            mp_dongusu_baslat(baglanti_soketi, kutu_baglanti["is_host"])
            return
        scanline_ciz(); ekrani_guncelle(); saat.tick(30)

def mp_dongusu_baslat(conn, is_host):
    """Seçili moda (MP_MOD) göre doğru oyun döngüsünü başlatır."""
    if MP_MOD == "futbol":
        mp_futbol_dongusu(conn, is_host)
    else:
        mp_oyun_dongusu(conn, is_host)
...
def mp_futbol_dongusu(conn, is_host):
    ...
    kutu = {'x': (EN - 100 if is_host else 100), 'y': 500, 'can': 100,
            'gelen_hasar': 0, 'rakip_oldu': False, 'baglanti_koptu': False,
            'top': None, 'gelen_top_atis': [], 'harita': None, 'gelen_top_vurus': 0}
    threading.Thread(target=mp_alici_thread, args=(conn, kutu), daemon=True).start()
