    ...
    try:
        async for ilk_mesaj in websocket:
            ...
            if tip == 'oda_kur':
                ...
                # kendi hareketleri sessizce yok sayılıyordu). Şimdi bir "hazır" sinyaline (event)
                # kadar bekliyor; sinyal gelince (ya da bağlantı koparsa) döngüden çıkıyor.
                event_gorevi = asyncio.create_task(odalar[oda_adi]['hazir_event'].wait())
                kapanma_gorevi = asyncio.create_task(websocket.wait_closed())
                bitenler, bekleyenler = await asyncio.wait(
                    [event_gorevi, kapanma_gorevi], return_when=asyncio.FIRST_COMPLETED)
                for g in bekleyenler:
                    g.cancel()
                if kapanma_gorevi in bitenler:
                    return  # bağlantı koptu, finally bloğu temizleyecek
                break  # rakip geldi, aktarım moduna geç
