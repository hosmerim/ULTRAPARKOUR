...
def fare_konumu():
    ...
    Aktif bir nişan-alma dokunuşu varsa (mobil), onu döndürür; yoksa gerçek fareyi kullanır."""
    if DOKUNMATIK["nisan_konum"] is not None:
        return DOKUNMATIK["nisan_konum"]
    mx, my = pygame.mouse.get_pos()
    olcek, ofset_x, ofset_y, _, _ = _olcek_bilgisi()
    return (int((mx - ofset_x) / olcek), int((my - ofset_y) / olcek))

# ============================================================
#  DOKUNMATİK (MOBİL) KONTROL SİSTEMİ
#  Telefon/tablette klavye+fare olmadığı için ekrana sanal butonlar
#  çiziyoruz. Sürekli basılan tuşlar (hareket, taramalı ateş) DOKUNMATIK
#  sözlüğünden okunuyor; tek seferlik tuşlar (zıplama, parry, silah
#  değiştirme, ateş/fırlatma) sentetik pygame olayı olarak gönderiliyor
#  (pygame.event.post) — böylece mevcut oyun kodu değiştirilmeden çalışıyor.
DOKUNMATIK = {"sol": False, "sag": False, "ates_basili": False, "nisan_konum": None, "parmaklar": {}}

def _dokunma_konumu(event):
    """FINGER olaylarının normalize (0..1) konumunu 800x600 canvas koordinatına çevirir."""
    ger_en, ger_boy = gercek_ekran.get_size()
    olcek, ofset_x, ofset_y, _, _ = _olcek_bilgisi()
    gx, gy = event.x * ger_en, event.y * ger_boy
    return (int((gx - ofset_x) / olcek), int((gy - ofset_y) / olcek))

def dokunmatik_buton_alanlari():
    """Sanal butonların ekran dikdörtgenlerini döndürür (çizim ve dokunma testi için ortak).
    Sol alt: hareket + zıplama. Sağ alt: parry + silah değiştirme. Kalan her yer: nişan al + ateş/fırlat."""
    return {
        "sol":   pygame.Rect(20, BOY - 110, 70, 70),
        "sag":   pygame.Rect(100, BOY - 110, 70, 70),
        "zipla": pygame.Rect(190, BOY - 110, 70, 70),
        "parry": pygame.Rect(EN - 100, BOY - 190, 70, 70),
        "s1":    pygame.Rect(EN - 260, BOY - 80, 50, 50),
        "s2":    pygame.Rect(EN - 200, BOY - 80, 50, 50),
        "s3":    pygame.Rect(EN - 140, BOY - 80, 50, 50),
    }

def dokunmatik_event_isle(event):
    """Ana oyun döngülerinin event for'u içinden çağrılır. FINGERDOWN/MOTION/UP olaylarını işler."""
    alanlar = dokunmatik_buton_alanlari()
    if event.type == pygame.FINGERDOWN:
        konum = _dokunma_konumu(event)
        nokta = pygame.Rect(konum[0]-1, konum[1]-1, 2, 2)
        if alanlar["sol"].colliderect(nokta):
            DOKUNMATIK["sol"] = True; DOKUNMATIK["parmaklar"][event.finger_id] = "sol"
        elif alanlar["sag"].colliderect(nokta):
            DOKUNMATIK["sag"] = True; DOKUNMATIK["parmaklar"][event.finger_id] = "sag"
        elif alanlar["zipla"].colliderect(nokta):
            DOKUNMATIK["parmaklar"][event.finger_id] = "zipla"
            pygame.event.post(pygame.event.Event(pygame.KEYDOWN, key=pygame.K_w))
        elif alanlar["parry"].colliderect(nokta):
            DOKUNMATIK["parmaklar"][event.finger_id] = "parry"
            pygame.event.post(pygame.event.Event(pygame.KEYDOWN, key=pygame.K_f))
        elif alanlar["s1"].colliderect(nokta):
            DOKUNMATIK["parmaklar"][event.finger_id] = "s1"
            pygame.event.post(pygame.event.Event(pygame.KEYDOWN, key=pygame.K_1))
        elif alanlar["s2"].colliderect(nokta):
            DOKUNMATIK["parmaklar"][event.finger_id] = "s2"
            pygame.event.post(pygame.event.Event(pygame.KEYDOWN, key=pygame.K_2))
        elif alanlar["s3"].colliderect(nokta):
            DOKUNMATIK["parmaklar"][event.finger_id] = "s3"
            pygame.event.post(pygame.event.Event(pygame.KEYDOWN, key=pygame.K_3))
        else:
            # Ekranın geri kalanı: nişan al + ateş/fırlat alanı
            DOKUNMATIK["parmaklar"][event.finger_id] = "nisan"
            DOKUNMATIK["nisan_konum"] = konum
            DOKUNMATIK["ates_basili"] = True
            pygame.event.post(pygame.event.Event(pygame.MOUSEBUTTONDOWN, button=1, pos=konum))
    elif event.type == pygame.FINGERMOTION:
        gorev = DOKUNMATIK["parmaklar"].get(event.finger_id)
        if gorev == "nisan":
            DOKUNMATIK["nisan_konum"] = _dokunma_konumu(event)
    elif event.type == pygame.FINGERUP:
        gorev = DOKUNMATIK["parmaklar"].pop(event.finger_id, None)
        if gorev == "sol": DOKUNMATIK["sol"] = False
        elif gorev == "sag": DOKUNMATIK["sag"] = False
        elif gorev == "nisan":
            DOKUNMATIK["ates_basili"] = False
            if "nisan" not in DOKUNMATIK["parmaklar"].values():
                DOKUNMATIK["nisan_konum"] = None

def dokunmatik_overlay_ciz(parry_var=True, silah_var=True):
    """Sanal kontrol butonlarını yarı saydam şekilde ekrana çizer.
    parry_var/silah_var: bazı modlarda (örn. futbol) bu butonlar gösterilmesin diye."""
    alanlar = dokunmatik_buton_alanlari()
    etiketler = {"sol": "◄", "sag": "►", "zipla": "▲", "parry": "F", "s1": "1", "s2": "2", "s3": "3"}
    gosterilecekler = ["sol", "sag", "zipla"]
    if parry_var: gosterilecekler.append("parry")
    if silah_var: gosterilecekler += ["s1", "s2", "s3"]
    for ad in gosterilecekler:
        r = alanlar[ad]
        aktif = (ad == "sol" and DOKUNMATIK["sol"]) or (ad == "sag" and DOKUNMATIK["sag"])
        renk = SARI if aktif else BEYAZ
        yuzey = pygame.Surface((r.w, r.h), pygame.SRCALPHA)
        pygame.draw.rect(yuzey, (255, 255, 255, 40), (0, 0, r.w, r.h), border_radius=10)
        pygame.draw.rect(yuzey, renk, (0, 0, r.w, r.h), 2, border_radius=10)
        ekran.blit(yuzey, (r.x, r.y))
        etiket = font_buton.render(etiketler[ad], True, renk)
        ekran.blit(etiket, (r.centerx - etiket.get_width()//2, r.centery - etiket.get_height()//2))
...
def oyun_dongusu():
    ...
    while True:
        ...
        if secili_silah == "taramali" and (pygame.mouse.get_pressed()[0] or DOKUNMATIK["ates_basili"]) and ates_cd == 0:
            fx, fy = fare_konumu()
            fx_dunya = fx + kam_x
            aci = math.atan2(fy - oyuncu.centery, fx_dunya - oyuncu.centerx)
        ...
        if tus[pygame.K_a] or DOKUNMATIK["sol"]: oyuncu.x -= 5; hareket_ediyor = True
        if tus[pygame.K_d] or DOKUNMATIK["sag"]: oyuncu.x += 5; hareket_ediyor = True
...
def mp_oyun_dongusu(conn, is_host, rakip_adi="RAKİP"):
    ...
    while True:
        ...
        # --- TARAMALI SİLAH: basılı tutarak sürekli ateş ---
        ates_cd_kalan = max(0, silah_beklemeleri.get(secili_silah, 3) - (su_an - son_ates))
        if kazandi is None and secili_silah == "taramali" and (pygame.mouse.get_pressed()[0] or DOKUNMATIK["ates_basili"]) and ates_cd_kalan == 0:
            fx, fy = fare_konumu()
            aci = math.atan2(fy - oyuncu.centery, fx - oyuncu.centerx)
        ...
        if kazandi is None:
            tus = pygame.key.get_pressed()
            if tus[pygame.K_a] or DOKUNMATIK["sol"]: oyuncu.x -= 5
            if tus[pygame.K_d] or DOKUNMATIK["sag"]: oyuncu.x += 5
...
def mp_futbol_dongusu(conn, is_host, rakip_adi="RAKİP"):
    ...
    while True:
        ...
        dokunmatik_overlay_ciz(parry_var=False, silah_var=False)
        scanline_ciz(); ekrani_guncelle(); saat.tick(60)
